"""This module defines functionality for samples/04-hosting/foundry-hosted-agents/invocations/telegram/tests/test_infrastructure.
...
def test_direct_code_invocations_2_configuration() -> None:
    """Validates behavior for direct code invocations 2 configuration.
    """
...
def test_dependencies_use_sample_local_pyproject() -> None:
    """Validates behavior for dependencies use sample local pyproject.
    """
...
def test_sensitive_telemetry_is_configurable_for_hosted_agent() -> None:
    """Validates behavior for sensitive telemetry is configurable for hosted agent.
    """
...
def test_agent_instructions_are_included_in_deployment() -> None:
    """Validates behavior for agent instructions are included in deployment.
    """
...
def test_apim_policy_preserves_update_and_injects_channel() -> None:
    """Validates behavior for apim policy preserves update and injects channel.
    """
...
def test_cosmos_partition_key_is_session_id() -> None:
    """Validates behavior for cosmos partition key is session id.
    """
...
def test_deployment_health_check_rejects_direct_invocation_and_is_subscription_scoped() -> None:
    """Validates behavior for deployment health check rejects direct invocation and is subscription scoped.
    """
    deploy_script = (SAMPLE_ROOT / "deploy.sh").read_text()
...
def test_deployment_and_removal_require_sample_owned_resource_group() -> None:
    """Validates behavior for deployment and removal require sample owned resource group.
    """
...
def test_default_model_is_gpt_5_6_luna() -> None:
    """Validates behavior for default model is gpt 5 6 luna.
    """
    deploy_script = (SAMPLE_ROOT / "deploy.sh").read_text()
...
def test_remove_script_unregisters_webhook_before_deleting_resource_group() -> None:
    """Validates behavior for remove script unregisters webhook before deleting resource group.
    """
