"""This module defines functionality for packages/hosting-a2a/tests/hosting_a2a/test_conversion.
...
@dataclass
class WorkflowInput:
    """Represents the WorkflowInput type and related behavior.
    """
    text: str
    repeat: int
...
def test_a2a_to_run_converts_supported_parts() -> None:
    """Validates behavior for a2a to run converts supported parts.
    """
    data_part = Part()
...
def test_a2a_to_run_rejects_empty_message() -> None:
    """Validates behavior for a2a to run rejects empty message.
    """
...
def test_a2a_to_run_omits_unsupported_parts() -> None:
    """Validates behavior for a2a to run omits unsupported parts.
    """
    run = a2a_to_run(
        A2AMessage(
            message_id="message-1",
...
def test_a2a_to_run_validates_advertised_input_modes() -> None:
    """Validates behavior for a2a to run validates advertised input modes.
    """
    message = A2AMessage(
        message_id="message-1",
        role=Role.ROLE_USER,
...
def test_a2a_from_run_converts_final_response() -> None:
    """Validates behavior for a2a from run converts final response.
    """
...
def test_a2a_from_run_converts_streaming_update() -> None:
    """Validates behavior for a2a from run converts streaming update.
    """
...
def test_a2a_from_run_preserves_empty_text_part() -> None:
    """Validates behavior for a2a from run preserves empty text part.
    """
...
def test_a2a_from_run_omits_unsupported_content() -> None:
    """Validates behavior for a2a from run omits unsupported content.
    """
    parts = a2a_from_run(
        Message(
            "assistant",
...
def test_a2a_from_run_omits_user_messages() -> None:
    """Validates behavior for a2a from run omits user messages.
    """
...
def test_a2a_from_run_validates_advertised_output_modes() -> None:
    """Validates behavior for a2a from run validates advertised output modes.
    """
    result = Message(
        "assistant",
        [Content.from_uri("https://example.com/image.png", media_type="image/png")],
...
def test_a2a_from_run_parses_json_text_for_json_output_mode() -> None:
    """Validates behavior for a2a from run parses json text for json output mode.
    """
    parts = a2a_from_run(
...
def test_a2a_from_run_rejects_text_for_incompatible_output_mode() -> None:
    """Validates behavior for a2a from run rejects text for incompatible output mode.
    """
...
def test_a2a_from_run_rejects_invalid_data_uri() -> None:
    """Validates behavior for a2a from run rejects invalid data uri.
    """
...
def test_a2a_from_run_rejects_invalid_base64_data() -> None:
    """Validates behavior for a2a from run rejects invalid base64 data.
    """
    content = Content(
        "data",
...
def test_a2a_to_workflow_run_validates_structured_input() -> None:
    """Validates behavior for a2a to workflow run validates structured input.
    """
    data_part = Part()
...
def test_a2a_to_workflow_run_rejects_invalid_structured_input() -> None:
    """Validates behavior for a2a to workflow run rejects invalid structured input.
    """
    data_part = Part()
...
def test_a2a_to_workflow_run_supports_text_and_binary_inputs() -> None:
    """Validates behavior for a2a to workflow run supports text and binary inputs.
    """
    @executor(id="text")
...
def test_a2a_to_workflow_run_requires_one_compatible_part() -> None:
    """Validates behavior for a2a to workflow run requires one compatible part.
    """
    @executor(id="text")
    async def text_input(value: str, ctx: WorkflowContext[object, str]) -> None:
        """Implements text input.
...
def test_a2a_from_workflow_run_converts_public_outputs() -> None:
    """Validates behavior for a2a from workflow run converts public outputs.
    """
...
def test_a2a_from_workflow_run_serializes_structured_output_for_text_mode() -> None:
    """Validates behavior for a2a from workflow run serializes structured output for text mode.
    """
...
def test_a2a_from_workflow_run_rejects_pending_input() -> None:
    """Validates behavior for a2a from workflow run rejects pending input.
    """
    result = WorkflowRunResult([
