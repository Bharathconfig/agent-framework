"""This module defines functionality for packages/hosting-a2a/tests/hosting_a2a/test_adapters.
...
class StructuredInput:
    """Represents the StructuredInput type and related behavior.
    """
    value: str


async def test_agent_card_infers_metadata_and_keeps_a2a_policy_explicit() -> None:
    """Validates behavior for agent card infers metadata and keeps a2a policy explicit.
    """
...
async def test_agent_card_requires_public_metadata_and_interface() -> None:
    """Validates behavior for agent card requires public metadata and interface.
    """
...
async def test_agent_card_infers_agent_framework_skills_and_can_disable_inference() -> None:
    """Validates behavior for agent card infers agent framework skills and can disable inference.
    """
    skill = InlineSkill(
...
async def test_workflow_card_infers_json_input_and_text_output_modes() -> None:
    """Validates behavior for workflow card infers json input and text output modes.
    """
...
async def test_workflow_card_allows_explicit_modes_for_unsupported_types() -> None:
    """Validates behavior for workflow card allows explicit modes for unsupported types.
    """
    @executor(id="custom")
