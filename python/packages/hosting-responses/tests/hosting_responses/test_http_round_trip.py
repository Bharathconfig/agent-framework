...
class TestNonStreamingRoundTrip:
    """Represents the TestNonStreamingRoundTrip type and related behavior.
    """
    async def test_returns_responses_shaped_payload(self) -> None:
        """Validates behavior for returns responses shaped payload.
    ...
    async def test_invalid_input_returns_400_not_500(self) -> None:
        """Validates behavior for invalid input returns 400 not 500.
    ...
    async def test_conflicting_continuation_mechanisms_return_400(self) -> None:
        """Validates behavior for conflicting continuation mechanisms return 400.
    ...
    async def test_missing_message_role_returns_400(self) -> None:
        """Validates behavior for missing message role returns 400.
...
class TestStreamingRoundTrip:
    """Represents the TestStreamingRoundTrip type and related behavior.
    """
    async def test_stream_emits_created_delta_and_completed_events(self) -> None:
        """Validates behavior for stream emits created delta and completed events.
...
class TestSessionContinuity:
    ...
    async def test_previous_response_id_chain_preserves_session_across_three_turns(self) -> None:
        """Validates behavior for previous response id chain preserves session across three turns.
    ...
    async def test_previous_response_id_supports_independent_branches(self) -> None:
        """Validates behavior for previous response id supports independent branches.
    ...
    async def test_conversation_preserves_session_across_turns(self) -> None:
        """Validates behavior for conversation preserves session across turns.
    ...
    async def test_unrelated_requests_get_independent_sessions(self) -> None:
        """Validates behavior for unrelated requests get independent sessions.
