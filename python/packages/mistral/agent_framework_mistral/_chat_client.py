"""This module defines functionality for packages/mistral/agent_framework_mistral/_chat_client.
...
def _function_call_content(tool_call: ToolCall) -> Content:
    """Implements  function call content.
...
class _StreamedToolCalls:
    ...
    def flush_choice(self, choice_index: int) -> list[Content]:
        """Implements flush choice.
        
        Args:
            self: Description of self.
            choice_index: Description of choice_index.
    ...
    def _key_for(self, choice_index: int, fragment: ToolCall) -> tuple[int, int | str]:
        ...
        Args:
            self: Description of self.
            choice_index: Description of choice_index.
            fragment: Description of fragment.
    ...
    def _merge(pending: ToolCall, fragment: ToolCall) -> None:
        ...
        Args:
            pending: Description of pending.
            fragment: Description of fragment.
        """
        if fragment_id := _tool_call_id_of(fragment):
...
class RawMistralChatClient(
    ...
    def _raise_sdk_error(ex: MistralError, *, streaming: bool = False) -> NoReturn:
        """Implements  raise sdk error.
        
        Args:
            ex: Description of ex.
    ...
    def _prepare_mistral_messages(self, messages: Sequence[Message]) -> list[dict[str, Any]]:
        """Implements  prepare mistral messages.
    ...
    def _format_user_message(self, message: Message) -> dict[str, Any]:
        """Implements  format user message.
    ...
    def _format_assistant_message(self, message: Message) -> dict[str, Any]:
        """Implements  format assistant message.
    ...
    def _format_tool_messages(self, message: Message) -> list[dict[str, Any]]:
        """Implements  format tool messages.
    ...
    def _parse_message_contents(self, message: AssistantMessage) -> list[Content]:
        """Implements  parse message contents.
    ...
    def _parse_content_chunks(self, message: AssistantMessage | DeltaMessage) -> list[Content]:
        """Implements  parse content chunks.
    ...
    def _format_created_at(created: Any) -> str | None:
        """Implements  format created at.
        
        Args:
            created: Description of created.
    ...
    def _parse_response_metadata(usage: UsageInfo | None) -> dict[str, Any] | None:
        """Implements  parse response metadata.
