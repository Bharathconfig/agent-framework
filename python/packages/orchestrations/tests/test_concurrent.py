"""This module defines functionality for packages/orchestrations/tests/test_concurrent.
...
def test_concurrent_builder_rejects_empty_participants() -> None:
    """Validates behavior for concurrent builder rejects empty participants.
    """
...
def test_concurrent_builder_rejects_duplicate_executors() -> None:
    """Validates behavior for concurrent builder rejects duplicate executors.
    """
...
async def test_concurrent_custom_aggregator_callback_is_used() -> None:
    ...
    """Validates behavior for concurrent custom aggregator callback is used.
...
async def test_concurrent_custom_aggregator_sync_callback_is_used() -> None:
    """Validates behavior for concurrent custom aggregator sync callback is used.
    ...
    def summarize_sync(results: list[AgentExecutorResponse], _ctx: WorkflowContext[Any]) -> str:  # type: ignore[unused-argument]
        """Implements summarize sync.
        
        Args:
            results: Description of results.
            _ctx: Description of _ctx.
...
def test_concurrent_custom_aggregator_uses_callback_name_for_id() -> None:
    """Validates behavior for concurrent custom aggregator uses callback name for id.
...
async def test_concurrent_with_aggregator_executor_instance() -> None:
    ...
    class CustomAggregator(Executor):
        """Represents the CustomAggregator type and related behavior.
        """
        @handler
...
async def test_concurrent_checkpoint_resume_round_trip() -> None:
    """Validates behavior for concurrent checkpoint resume round trip.
    """
