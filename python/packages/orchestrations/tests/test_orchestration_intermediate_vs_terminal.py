...
async def test_sequential_intermediate_output_from_surface_as_intermediate() -> None:
    """Validates behavior for sequential intermediate output from surface as intermediate.
    """
    a = _EchoAgent(name="A")
    b = _EchoAgent(name="B")
    c = _EchoAgent(name="C")
...
async def test_concurrent_intermediate_output_from_surface_as_intermediate() -> None:
    """Validates behavior for concurrent intermediate output from surface as intermediate.
    """
    a = _EchoAgent(name="A")
    b = _EchoAgent(name="B")
...
async def test_group_chat_intermediate_output_from_surface_as_intermediate() -> None:
    """Validates behavior for group chat intermediate output from surface as intermediate.
    """
    alpha = _EchoAgent(name="alpha")
    beta = _EchoAgent(name="beta")
...
def test_handoff_builder_designates_every_participant_as_output() -> None:
    ...
    class _StubClient(FunctionInvocationLayer[Any], ChatMiddlewareLayer[Any], BaseChatClient[Any]):
        """Represents the _StubClient type and related behavior.
        """
...
def test_handoff_builder_output_from_can_select_workflow_output_participants() -> None:
    """Validates behavior for handoff builder output from can select workflow output participants.
    """
    from agent_framework import Agent
    from agent_framework._clients import BaseChatClient
    from agent_framework._middleware import ChatMiddlewareLayer
...
def test_magentic_builder_intermediate_output_from_designates_intermediate_workers() -> None:
    """Validates behavior for magentic builder intermediate output from designates intermediate workers.
    """
    manager = _StubMagenticManager()
    alpha = _EchoAgent(name="alpha")
...
def test_sequential_output_from_all_selects_all_participants() -> None:
    """Validates behavior for sequential output from all selects all participants.
    """
    a = _EchoAgent(name="A")
    b = _EchoAgent(name="B")
    c = _EchoAgent(name="C")
...
def test_sequential_intermediate_output_from_all_other_selects_non_outputs() -> None:
    """Validates behavior for sequential intermediate output from all other selects non outputs.
    """
    a = _EchoAgent(name="A")
    b = _EchoAgent(name="B")
    c = _EchoAgent(name="C")
...
def test_sequential_all_other_with_omitted_output_from_selects_all_intermediate() -> None:
    """Validates behavior for sequential all other with omitted output from selects all intermediate.
    """
    a = _EchoAgent(name="A")
    b = _EchoAgent(name="B")
...
def _build_sequential_with_designation(**kwargs: Any) -> None:
    """Implements  build sequential with designation.
...
def _build_concurrent_with_designation(**kwargs: Any) -> None:
    """Implements  build concurrent with designation.
...
def _build_group_chat_with_designation(**kwargs: Any) -> None:
    """Implements  build group chat with designation.
...
def _build_magentic_with_designation(**kwargs: Any) -> None:
    """Implements  build magentic with designation.
...
def _build_handoff_with_designation(**kwargs: Any) -> None:
    """Implements  build handoff with designation.
...
def test_participant_output_config_validation(build: Callable[..., None], kwargs: dict[str, Any], match: str) -> None:
    """Validates behavior for participant output config validation.
    
    Args:
        build: Description of build.
        kwargs: Description of kwargs.
...
def test_participant_output_config_rejects_final_output_from_parameter(build: Callable[..., None]) -> None:
    """Validates behavior for participant output config rejects final output from parameter.
