"""This module defines functionality for packages/orchestrations/tests/test_magentic.
...
def test_magentic_context_reset_behavior():
    """Validates behavior for magentic context reset behavior.
    """
    ctx = MagenticContext(
        task="task",
...
@dataclass
class _SimpleLedger:
    """Represents the _SimpleLedger type and related behavior.
    """
    facts: Message
    plan: Message
...
class FakeManager(MagenticManagerBase):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            max_stall_count: Description of max_stall_count.
            max_reset_count: Description of max_reset_count.
            max_round_count: Description of max_round_count.
        """
        super().__init__(
    ...
    def on_checkpoint_restore(self, state: dict[str, Any]) -> None:
        """Implements on checkpoint restore.
    ...
    async def plan(self, magentic_context: MagenticContext) -> Message:
        ...
        Args:
            self: Description of self.
            magentic_context: Description of magentic_context.
    ...
    async def create_progress_ledger(self, magentic_context: MagenticContext) -> MagenticProgressLedger:
        ...
        """Implements create progress ledger.
    ...
    async def prepare_final_answer(self, magentic_context: MagenticContext) -> Message:
        """Implements prepare final answer.
...
class StubAgent(BaseAgent):
    """Represents the StubAgent type and related behavior.
    """
    def __init__(self, agent_name: str, reply_text: str, **kwargs: Any) -> None:
        ...
        Args:
            self: Description of self.
            agent_name: Description of agent_name.
            reply_text: Description of reply_text.
...
class DummyExec(Executor):
    """Represents the DummyExec type and related behavior.
    """
...
def test_magentic_builder_marks_feature_with_custom_manager() -> None:
    """Validates behavior for magentic builder marks feature with custom manager.
    """
...
async def test_magentic_builder_returns_workflow_and_runs() -> None:
    """Validates behavior for magentic builder returns workflow and runs.
    """
    manager = FakeManager()
...
async def test_magentic_as_agent_does_not_accept_conversation() -> None:
    """Validates behavior for magentic as agent does not accept conversation.
    """
    manager = FakeManager()
...
async def test_standard_manager_plan_and_replan_combined_ledger():
    """Validates behavior for standard manager plan and replan combined ledger.
    """
    manager = FakeManager()
    ctx = MagenticContext(
        task="demo task",
...
async def test_magentic_workflow_plan_review_approval_to_completion():
    """Validates behavior for magentic workflow plan review approval to completion.
    """
    manager = FakeManager()
...
async def test_magentic_plan_review_with_revise():
    """Validates behavior for magentic plan review with revise.
    ...
    class CountingManager(FakeManager):
        ...
        """Represents the CountingManager type and related behavior.
        """
        replan_count: int = 0
...
async def test_magentic_orchestrator_round_limit_produces_partial_result():
    """Validates behavior for magentic orchestrator round limit produces partial result.
    """
    manager = FakeManager(max_round_count=1)
...
async def test_magentic_checkpoint_resume_round_trip():
    """Validates behavior for magentic checkpoint resume round trip.
    """
...
async def test_standard_manager_plan_and_replan_via_complete_monkeypatch():
    """Validates behavior for standard manager plan and replan via complete monkeypatch.
    ...
    async def fake_complete_replan(messages: list[Message], **kwargs: Any) -> Message:
        """Implements fake complete replan.
...
async def test_standard_manager_progress_ledger_success_and_error():
    """Validates behavior for standard manager progress ledger success and error.
...
class InvokeOnceManager(MagenticManagerBase):
    """Represents the InvokeOnceManager type and related behavior.
    """
...
class StubThreadAgent(BaseAgent):
    """Represents the StubThreadAgent type and related behavior.
    """
...
class StubAssistantsClient:
    """Represents the StubAssistantsClient type and related behavior.
    """
    pass  # class name used for branch detection


class StubAssistantsAgent(BaseAgent):
    """Represents the StubAssistantsAgent type and related behavior.
    """
...
async def _collect_agent_responses_setup(participant: SupportsAgentRun) -> list[Message]:
    """Implements  collect agent responses setup.
...
async def test_agent_executor_invoke_with_thread_chat_client():
    """Validates behavior for agent executor invoke with thread chat client.
    """
...
async def test_agent_executor_invoke_with_assistants_client_messages():
    """Validates behavior for agent executor invoke with assistants client messages.
    """
...
async def _collect_checkpoints(
    ...
    """Implements  collect checkpoints.
    
    Args:
        storage: Description of storage.
        workflow_name: Description of workflow_name.
...
async def test_magentic_checkpoint_resume_inner_loop_superstep():
    """Validates behavior for magentic checkpoint resume inner loop superstep.
    """
    storage = InMemoryCheckpointStorage()
...
async def test_magentic_checkpoint_resume_rejects_participant_renames():
    """Validates behavior for magentic checkpoint resume rejects participant renames.
    """
    storage = InMemoryCheckpointStorage()
...
async def test_magentic_stall_and_reset_reach_limits():
    """Validates behavior for magentic stall and reset reach limits.
    """
...
def test_magentic_with_custom_manager_does_not_warn_without_standard_manager_options(caplog: Any) -> None:
    """Validates behavior for magentic with custom manager does not warn without standard manager options.
...
def test_magentic_with_custom_manager_factory_does_not_warn_without_standard_manager_options(caplog: Any) -> None:
    """Validates behavior for magentic with custom manager factory does not warn without standard manager options.
...
def test_magentic_with_custom_manager_warns_when_standard_manager_option_is_provided(caplog: Any) -> None:
    """Validates behavior for magentic with custom manager warns when standard manager option is provided.
...
def test_magentic_agent_factory_uses_default_max_stall_count() -> None:
    """Validates behavior for magentic agent factory uses default max stall count.
