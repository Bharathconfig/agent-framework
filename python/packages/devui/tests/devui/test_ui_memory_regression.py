...
class _BrowserProcessRow:
    """Represents the _BrowserProcessRow type and related behavior.
    """
    pid: int
    parent_pid: int
    rss_kb: int
...
class _BrowserMemoryProbe:
    """Represents the _BrowserMemoryProbe type and related behavior.
    """
...
class MemoryStressAgent(BaseAgent):
    ...
    def __init__(self, *, chunk_count: int, chunk_size: int, delay_ms: float, **kwargs: Any) -> None:
        ...
        Args:
            ...
            chunk_count: Description of chunk_count.
            chunk_size: Description of chunk_size.
            delay_ms: Description of delay_ms.
...
def _get_browser_candidates() -> tuple[Path, ...]:
    """Implements  get browser candidates.
...
def _find_browser_executable() -> Path | None:
    """Implements  find browser executable.
...
def _find_available_port() -> int:
    """Implements  find available port.
...
async def _get_devtools_websocket_url(port: int) -> str:
    """Implements  get devtools websocket url.
...
def _wait_for_server_details(server_instance: uvicorn.Server) -> tuple[int, str]:
    """Implements  wait for server details.
    
    Args:
        server_instance: Description of server_instance.
...
def _parse_posix_process_rows(output: str) -> list[_BrowserProcessRow]:
    """Implements  parse posix process rows.
...
def _parse_windows_process_rows(output: str) -> list[_BrowserProcessRow]:
    """Implements  parse windows process rows.
...
def _collect_process_tree(root_pids: set[int], process_rows: list[_BrowserProcessRow]) -> list[_BrowserProcessRow]:
    """Implements  collect process tree.
    
    Args:
        root_pids: Description of root_pids.
        process_rows: Description of process_rows.
...
def _collect_browser_process_rows(root_pid: int, profile_dir: str) -> list[_BrowserProcessRow]:
    """Implements  collect browser process rows.
    
    Args:
        root_pid: Description of root_pid.
...
def _sample_peak_renderer_rss_mb(root_pid: int, profile_dir: str) -> float:
    """Implements  sample peak renderer rss mb.
...
async def _sample_browser_memory_probe(client: _CDPClient, *, session_id: str) -> _BrowserMemoryProbe:
    """Implements  sample browser memory probe.
...
def _terminate_browser_processes(root_pid: int, profile_dir: str) -> None:
    """Implements  terminate browser processes.
...
def _launch_browser_process(*, browser_path: Path, debug_port: int, profile_dir: str) -> subprocess.Popen[str]:
    """Implements  launch browser process.
    
    Args:
        browser_path: Description of browser_path.
        debug_port: Description of debug_port.
        profile_dir: Description of profile_dir.
...
def _shutdown_browser_process(browser_process: subprocess.Popen[str], *, profile_dir: str) -> None:
    """Implements  shutdown browser process.
    
    Args:
        browser_process: Description of browser_process.
        profile_dir: Description of profile_dir.
    """
    with contextlib.suppress(Exception):
...
def test_parse_posix_process_rows() -> None:
    """Validates behavior for parse posix process rows.
    """
    output = """
...
def test_parse_windows_process_rows() -> None:
    """Validates behavior for parse windows process rows.
    """
    output = json.dumps([
        {
            "pid": 301,
...
def test_sample_peak_renderer_rss_mb_uses_browser_process_tree(
    ...
    """Validates behavior for sample peak renderer rss mb uses browser process tree.
...
async def _wait_for_expression(
    ...
    *,
    ...
    """Implements  wait for expression.
    
    Args:
        client: Description of client.
        session_id: Description of session_id.
        expression: Description of expression.
        timeout_s: Description of timeout_s.
