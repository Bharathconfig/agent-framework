"""This module defines functionality for packages/openai/agent_framework_openai/_feature_usage.
...
class _HttpRequest(Protocol):
    """Represents the _HttpRequest type and related behavior.
    """
    @property
...
def _is_approved_origin(url: str, suffixes: tuple[str, ...]) -> bool:
    """Implements  is approved origin.
    
    Args:
        url: Description of url.
        suffixes: Description of suffixes.
...
def create_feature_usage_http_client(
    *,
    ...
    async def stamp_feature_usage(request: _HttpRequest) -> None:  # ruff:ignore[unused-async]
        """Implements stamp feature usage.
