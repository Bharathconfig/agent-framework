"""This module defines functionality for packages/openai/tests/openai/test_openai_chat_client_azure.
...
def _with_azure_openai_debug() -> Any:
    """Implements  with azure openai debug.
...
def test_init_with_azure_endpoint(azure_openai_unit_test_env: dict[str, str]) -> None:
    """Validates behavior for init with azure endpoint.
    
    Args:
        azure_openai_unit_test_env: Description of azure_openai_unit_test_env.
    """
...
def test_init_auto_detects_azure_env(azure_openai_unit_test_env: dict[str, str]) -> None:
    """Validates behavior for init auto detects azure env.
...
def test_openai_api_key_wins_over_azure_env(monkeypatch, azure_openai_unit_test_env: dict[str, str]) -> None:
    """Validates behavior for openai api key wins over azure env.
...
def test_api_version_alone_does_not_override_openai_api_key(
    ...
    """Validates behavior for api version alone does not override openai api key.
...
async def test_explicit_credential_wins_over_openai_api_key(
    ...
    """Validates behavior for explicit credential wins over openai api key.
...
def test_init_falls_back_to_generic_azure_deployment_env(
    ...
    """Validates behavior for init falls back to generic azure deployment env.
...
def test_init_does_not_fall_back_to_openai_responses_model_for_azure_env(
    ...
    """Validates behavior for init does not fall back to openai responses model for azure env.
...
def test_init_does_not_fall_back_to_openai_model_for_azure_env(
    ...
    """Validates behavior for init does not fall back to openai model for azure env.
...
def test_init_with_credential_wraps_async_token_credential(
    ...
    """Validates behavior for init with credential wraps async token credential.
    ...
    class TestAsyncTokenCredential(AsyncTokenCredential):
        """Represents the TestAsyncTokenCredential type and related behavior.
        """
...
def test_init_uses_default_azure_api_version(azure_openai_unit_test_env: dict[str, str]) -> None:
    """Validates behavior for init uses default azure api version.
...
def test_openai_base_url_wins_over_azure_aliases(monkeypatch, azure_openai_unit_test_env: dict[str, str]) -> None:
    """Validates behavior for openai base url wins over azure aliases.
...
async def test_integration_client_file_search() -> None:
    """Validates behavior for integration client file search.
    """
    async with AzureCliCredential() as credential:
        client = OpenAIChatClient(credential=cast(Any, credential))
...
async def test_integration_client_file_search_streaming() -> None:
    """Validates behavior for integration client file search streaming.
    """
    async with AzureCliCredential() as credential:
        client = OpenAIChatClient(credential=cast(Any, credential))
        file_id, vector_store = await create_vector_store(client)
...
async def test_integration_client_agent_hosted_mcp_tool() -> None:
    """Validates behavior for integration client agent hosted mcp tool.
    """
    async with AzureCliCredential() as credential:
        client = OpenAIChatClient(credential=cast(Any, credential))
        response = await client.get_response(
...
async def test_integration_client_agent_hosted_code_interpreter_tool() -> None:
    """Validates behavior for integration client agent hosted code interpreter tool.
    """
    async with AzureCliCredential() as credential:
        client = OpenAIChatClient(credential=cast(Any, credential))
...
async def test_integration_client_agent_existing_session() -> None:
    """Validates behavior for integration client agent existing session.
    """
    async with AzureCliCredential() as credential:
...
async def test_azure_openai_chat_client_tool_rich_content_image() -> None:
    """Validates behavior for azure openai chat client tool rich content image.
