"""This module defines functionality for packages/openai/tests/openai/test_openai_chat_client.
...
class _FakeAsyncEventStream:
    """Represents the _FakeAsyncEventStream type and related behavior.
    """
...
def _as_chat_response_stream(
    ...
    """Implements  as chat response stream.
...
def _response_id_from_token(token: Any) -> str:
    """Implements  response id from token.
...
class _FakeAsyncEventStreamContext(_FakeAsyncEventStream):
    """Represents the _FakeAsyncEventStreamContext type and related behavior.
    """
...
def test_init(openai_unit_test_env: dict[str, str]) -> None:
    ...
    """Validates behavior for init.
    
    Args:
        openai_unit_test_env: Description of openai_unit_test_env.
    """
...
def test_init_uses_explicit_parameters() -> None:
    """Validates behavior for init uses explicit parameters.
    """
...
def test_raw_openai_chat_client_init_uses_explicit_parameters() -> None:
    """Validates behavior for raw openai chat client init uses explicit parameters.
    """
...
def test_agent_accepts_openai_chat_clients() -> None:
    """Validates behavior for agent accepts openai chat clients.
    """
...
def test_openai_chat_client_supports_all_tool_protocols() -> None:
    """Validates behavior for openai chat client supports all tool protocols.
    """
...
def test_protocol_isinstance_with_openai_chat_client_instance() -> None:
    """Validates behavior for protocol isinstance with openai chat client instance.
    """
...
def test_openai_chat_client_tool_methods_return_dict() -> None:
    """Validates behavior for openai chat client tool methods return dict.
    """
...
def test_init_prefers_openai_chat_model(monkeypatch, openai_unit_test_env: dict[str, str]) -> None:
    """Validates behavior for init prefers openai chat model.
...
def test_init_validation_fail() -> None:
    # Test successful initialization
    """Validates behavior for init validation fail.
    """
...
def test_init_model_constructor(openai_unit_test_env: dict[str, str]) -> None:
    # Test successful initialization
    """Validates behavior for init model constructor.
...
def test_init_with_default_header(openai_unit_test_env: dict[str, str]) -> None:
    """Validates behavior for init with default header.
...
def test_init_with_empty_model(openai_unit_test_env: dict[str, str]) -> None:
    """Validates behavior for init with empty model.
...
def test_init_with_empty_api_key(openai_unit_test_env: dict[str, str]) -> None:
    """Validates behavior for init with empty api key.
...
def test_serialize(openai_unit_test_env: dict[str, str]) -> None:
    """Validates behavior for serialize.
...
def test_serialize_with_org_id(openai_unit_test_env: dict[str, str]) -> None:
    """Validates behavior for serialize with org id.
...
class _FakeTelemetryStreamWrapper:
    ...
    * because ``with_raw_response.create()`` routes through the instrumented
    ...
    def __init__(self, stream_async_iter: Any) -> None:
        ...
        Args:
            self: Description of self.
            stream_async_iter: Description of stream_async_iter.
        """
...
async def test_streamed_reasoning_function_group_survives_serialization_for_stateless_replay() -> None:
    """Validates behavior for streamed reasoning function group survives serialization for stateless replay.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
...
def test_streamed_reasoning_text_replays_as_reasoning_content() -> None:
    """Validates behavior for streamed reasoning text replays as reasoning content.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
...
def _streamed_reasoning_text_events(event_kind: str) -> tuple[list[object], str, str, str]:
    """Implements  streamed reasoning text events.
    
    Args:
        event_kind: Description of event_kind.
...
def test_streamed_encrypted_reasoning_without_visible_text_is_replayable() -> None:
    """Validates behavior for streamed encrypted reasoning without visible text is replayable.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
    reasoning_done = MagicMock()
    reasoning_done.type = "response.output_item.done"
...
def test_streamed_summary_and_reasoning_text_replay_as_one_provider_item() -> None:
    """Validates behavior for streamed summary and reasoning text replay as one provider item.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
    function_call_ids: dict[int, tuple[str, str]] = {}
    seen_reasoning_delta_item_ids: set[str] = set()
...
def test_streaming_refusal_delta_creates_marked_text() -> None:
    """Validates behavior for streaming refusal delta creates marked text.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
    event = MagicMock()
...
def test_prepare_marked_refusal_text_uses_native_assistant_shape_and_input_text_fallback() -> None:
    """Validates behavior for prepare marked refusal text uses native assistant shape and input text fallback.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
...
async def test_non_streaming_reasoning_function_group_round_trips_for_stateless_replay() -> None:
    """Validates behavior for non streaming reasoning function group round trips for stateless replay.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
...
async def test_prepare_request_does_not_duplicate_encrypted_reasoning_include() -> None:
    """Validates behavior for prepare request does not duplicate encrypted reasoning include.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
...
async def test_stateless_reasoning_group_without_encrypted_content_is_rejected_before_transport() -> None:
    """Validates behavior for stateless reasoning group without encrypted content is rejected before transport.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
    create = AsyncMock()
    messages = [
...
async def test_partially_compacted_reasoning_group_is_rejected_before_transport() -> None:
    """Validates behavior for partially compacted reasoning group is rejected before transport.
    
    Returns:
        Description of the return value.
    """
    async def exclude_reasoning_message(messages: list[Message]) -> bool:
        """Implements exclude reasoning message.
...
async def test_split_reasoning_group_without_encrypted_content_is_rejected_before_transport() -> None:
    """Validates behavior for split reasoning group without encrypted content is rejected before transport.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
    create = AsyncMock()
    messages = [
...
async def test_fully_compacted_reasoning_group_continues_with_remaining_messages() -> None:
    """Validates behavior for fully compacted reasoning group continues with remaining messages.
    
    Returns:
        Description of the return value.
    """
    async def exclude_reasoning_group(messages: list[Message]) -> bool:
        """Implements exclude reasoning group.
...
async def test_encrypted_reasoning_capability_rejection_is_not_retried_lossily() -> None:
    """Validates behavior for encrypted reasoning capability rejection is not retried lossily.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
...
async def test_stateless_request_rejects_non_replayable_reasoning_bound_mcp_output() -> None:
    """Validates behavior for stateless request rejects non replayable reasoning bound mcp output.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
    messages = [
...
def test_parse_chunk_from_openai_function_call_is_actionable() -> None:
    """Validates behavior for parse chunk from openai function call is actionable.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
    chat_options: dict[str, Any] = {}
    function_call_ids: dict[int, tuple[str, str]] = {}
...
def test_parse_chunk_from_openai_custom_tool_call_done_is_informational_only() -> None:
    """Validates behavior for parse chunk from openai custom tool call done is informational only.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
    chat_options: dict[str, Any] = {}
    function_call_ids: dict[int, tuple[str, str]] = {}
...
def _make_url_citation_event(
    *,
    ...
    """Implements  make url citation event.
    
    Args:
        title: Description of title.
        get_url: Description of get_url.
...
def _make_mcp_call_done_event(output: str) -> MagicMock:
    """Implements  make mcp call done event.
...
def _make_azure_ai_search_output_event(
    ...
    *,
    ...
    """Implements  make azure ai search output event.
    
    Args:
        output: Description of output.
        event_type: Description of event_type.
        top_level_output: Description of top_level_output.
...
def test_prepare_content_for_openai_preserves_supported_function_output(
    ...
    """Validates behavior for prepare content for openai preserves supported function output.
...
def test_prepare_content_for_openai_normalizes_unsupported_falsey_function_output(output: Any, expected: str) -> None:
    """Validates behavior for prepare content for openai normalizes unsupported falsey function output.
...
async def test_prepare_options_store_parameter_handling() -> None:
    """Validates behavior for prepare options store parameter handling.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
    messages = [Message(role="user", contents=["Test message"])]
...
async def test_prepare_options_store_false_rejects_non_replayable_reasoning_items() -> None:
    """Validates behavior for prepare options store false rejects non replayable reasoning items.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
    messages = [
        Message(role="user", contents=[Content.from_text(text="search for hotels")]),
...
async def test_prepare_options_with_conversation_id_strips_server_items_for_mixed_history_and_live() -> None:
    """Validates behavior for prepare options with conversation id strips server items for mixed history and live.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
    messages = [
        Message(role="user", contents=[Content.from_text(text="search for hotels")]),
...
def _create_mock_responses_text_response(*, response_id: str) -> MagicMock:
    """Implements  create mock responses text response.
...
async def test_integration_web_search() -> None:
    """Validates behavior for integration web search.
    """
...
async def test_integration_file_search() -> None:
    """Validates behavior for integration file search.
    """
    openai_responses_client = OpenAIChatClient()
...
async def test_integration_streaming_file_search() -> None:
    """Validates behavior for integration streaming file search.
    """
    openai_responses_client = OpenAIChatClient()
...
def test_prepare_messages_for_openai_replays_completed_reasoning_bound_mcp_call() -> None:
    """Validates behavior for prepare messages for openai replays completed reasoning bound mcp call.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
...
def test_prepare_messages_for_openai_replays_active_mcp_call_across_reasoning_messages() -> None:
    """Validates behavior for prepare messages for openai replays active mcp call across reasoning messages.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
...
def test_prepare_messages_for_openai_replays_all_mcp_calls_for_one_reasoning_item() -> None:
    """Validates behavior for prepare messages for openai replays all mcp calls for one reasoning item.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
...
def _strip_rule_messages() -> list[Message]:
    """Implements  strip rule messages.
...
def test_stateless_history_preserves_pending_hosted_approval_request_until_response() -> None:
    """Validates behavior for stateless history preserves pending hosted approval request until response.
    """
    client = OpenAIChatClient(model="test-model", api_key="test-key")
    function_call = Content.from_function_call(
        call_id="mcp_pending",
...
def _breakpoint_text_content() -> Content:
    """Implements  breakpoint text content.
