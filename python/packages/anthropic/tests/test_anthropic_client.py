"""This module defines functionality for packages/anthropic/tests/test_anthropic_client.
...
def test_agent_accepts_anthropic_clients() -> None:
    """Validates behavior for agent accepts anthropic clients.
    """
...
def test_prepare_message_for_anthropic_provider_reasoning_without_signature_is_text(
    ...
    """Validates behavior for prepare message for anthropic provider reasoning without signature is text.
    
    Args:
        mock_anthropic_client: Description of mock_anthropic_client.
    """
    client = create_test_anthropic_client(mock_anthropic_client)
    message = Message(
        role="assistant",
...
def test_prepare_message_for_anthropic_attaches_signature_only_reasoning(
    ...
    """Validates behavior for prepare message for anthropic attaches signature only reasoning.
...
def test_prepare_message_for_anthropic_skips_orphan_signature_only_reasoning(
    ...
    """Validates behavior for prepare message for anthropic skips orphan signature only reasoning.
...
def _anthropic_status_error(
    ...
    """Implements  anthropic status error.
    
    Args:
        error_cls: Description of error_cls.
...
async def test_inner_get_response_streaming_wraps_sdk_errors(mock_anthropic_client: MagicMock) -> None:
    ...
    async def _raise_after_first_event() -> Any:
        """Implements  raise after first event.
        """
        event = MagicMock()
...
def _usage_message_start_event(*, input_tokens: int, output_tokens: int) -> MagicMock:
    """Implements  usage message start event.
    
    Args:
        input_tokens: Description of input_tokens.
        output_tokens: Description of output_tokens.
...
def _usage_message_delta_event(*, output_tokens: int, input_tokens: int | None = None) -> MagicMock:
    """Implements  usage message delta event.
...
def test_prepare_response_format_pydantic_model(
    ...
    class TestModel(BaseModel):
        """Represents the TestModel type and related behavior.
        """
        name: str
        age: int
...
async def test_prepare_options_uses_output_config_for_response_format(
    ...
    class StructuredOut(BaseModel):
        """Represents the StructuredOut type and related behavior.
        """
        answer: str
