...
async def test_probe_collects_shell_version_cwd_and_tools() -> None:
    """Validates behavior for probe collects shell version cwd and tools.
    """
...
async def test_probe_falls_back_to_stderr_for_version_when_stdout_empty() -> None:
    """Validates behavior for probe falls back to stderr for version when stdout empty.
    """
    executor = _FakeExecutor({
...
async def test_probe_timeout_yields_none_field_not_exception() -> None:
    """Validates behavior for probe timeout yields none field not exception.
    """
    executor = _FakeExecutor({
...
async def test_probe_swallows_expected_executor_failures() -> None:
    """Validates behavior for probe swallows expected executor failures.
    """
    executor = _FakeExecutor({
...
async def test_unexpected_exception_propagates() -> None:
    """Validates behavior for unexpected exception propagates.
    """
...
async def test_invalid_tool_name_is_rejected_before_probing() -> None:
    """Validates behavior for invalid tool name is rejected before probing.
    """
    executor = _FakeExecutor({
        "echo": _ok(stdout="VERSION=5\nCWD=/r\n"),
    })
...
async def test_duplicate_tools_are_deduplicated_case_insensitively() -> None:
    """Validates behavior for duplicate tools are deduplicated case insensitively.
    """
    executor = _FakeExecutor({
        "echo": _ok(stdout="VERSION=5\nCWD=/r\n"),
...
async def test_failed_probe_does_not_poison_subsequent_calls() -> None:
    """Validates behavior for failed probe does not poison subsequent calls.
    ...
    class Flaky:
        """Represents the Flaky type and related behavior.
        """
        start_calls = 0
...
async def test_concurrent_first_callers_share_a_single_probe() -> None:
    """Validates behavior for concurrent first callers share a single probe.
...
async def test_before_run_extends_instructions() -> None:
    """Validates behavior for before run extends instructions.
    """
    executor = _FakeExecutor({
        "echo": _ok(stdout="VERSION=5.2.21\nCWD=/repo\n"),
        "git --version": _ok(stdout="git version 2.40.0\n"),
    ...
    class FakeContext:
        """Represents the FakeContext type and related behavior.
        """
...
async def test_default_formatter_powershell_block_uses_pwsh_idioms() -> None:
    """Validates behavior for default formatter powershell block uses pwsh idioms.
    """
...
async def test_custom_formatter_is_used_when_provided() -> None:
    """Validates behavior for custom formatter is used when provided.
    """
    executor = _FakeExecutor({
        "echo": _ok(stdout="VERSION=5\nCWD=/r\n"),
    })
