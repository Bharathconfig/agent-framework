"""This module defines functionality for packages/tools/tests/test_local_shell_tool.
...
class _FakeExecProcess:
    """Represents the _FakeExecProcess type and related behavior.
    """
    def __init__(
        self,
        *,
...
async def test_stateless_echo() -> None:
    """Validates behavior for stateless echo.
    """
...
async def test_stateless_exit_code_propagates() -> None:
    """Validates behavior for stateless exit code propagates.
    """
    tool = LocalShellTool(mode="stateless", approval_mode="never_require", acknowledge_unsafe=True)
...
async def test_stateless_timeout_kills_long_command() -> None:
    """Validates behavior for stateless timeout kills long command.
    """
...
async def test_policy_denies_before_execution() -> None:
    """Validates behavior for policy denies before execution.
    """
    tool = LocalShellTool(
        mode="stateless",
...
async def test_allowlist_narrows_to_approved_commands() -> None:
    """Validates behavior for allowlist narrows to approved commands.
    """
    tool = LocalShellTool(
        mode="stateless",
        approval_mode="never_require",
...
async def test_audit_hook_fires_for_allowed_commands() -> None:
    """Validates behavior for audit hook fires for allowed commands.
    """
    seen: list[str] = []
    tool = LocalShellTool(
        mode="stateless",
...
def test_local_shell_tool_handles_mode_and_environment_variants(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for local shell tool handles mode and environment variants.
...
async def test_local_shell_tool_stateless_start_is_noop() -> None:
    """Validates behavior for local shell tool stateless start is noop.
    """
    tool = LocalShellTool(mode="stateless", approval_mode="never_require", acknowledge_unsafe=True)
    await tool.start()
    await tool.close()
...
async def test_local_shell_tool_raises_if_start_did_not_create_session() -> None:
    """Validates behavior for local shell tool raises if start did not create session.
    """
...
async def test_local_shell_tool_as_function_returns_policy_errors() -> None:
    """Validates behavior for local shell tool as function returns policy errors.
    """
    tool = LocalShellTool(mode="persistent", approval_mode="never_require", acknowledge_unsafe=True)
...
def test_local_shell_tool_reanchors_powershell_paths() -> None:
    """Validates behavior for local shell tool reanchors powershell paths.
    """
    tool = LocalShellTool(
        mode="persistent",
        shell="pwsh",
...
def test_popen_kwargs_for_group_covers_windows_branch(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for popen kwargs for group covers windows branch.
...
async def test_run_stateless_adds_powershell_encoding_preamble() -> None:
    """Validates behavior for run stateless adds powershell encoding preamble.
    """
...
async def test_run_stateless_timeout_returns_empty_output_if_drain_fails() -> None:
    """Validates behavior for run stateless timeout returns empty output if drain fails.
    """
...
async def test_persistent_preserves_cwd_and_exports_across_calls(tmp_path: os.PathLike[str]) -> None:
    """Validates behavior for persistent preserves cwd and exports across calls.
...
async def test_persistent_preserves_state_powershell(tmp_path: os.PathLike[str]) -> None:
    """Validates behavior for persistent preserves state powershell.
...
async def test_as_function_wires_kind_and_approval() -> None:
    """Validates behavior for as function wires kind and approval.
    """
