...
class _FakeProcess:
    """Represents the _FakeProcess type and related behavior.
    """
    def __init__(
        self,
        *,
        ...
        Args:
            ...
            pid: Description of pid.
            returncode: Description of returncode.
            communicate_results: Description of communicate_results.
            wait_results: Description of wait_results.
        """
        self.pid = pid
...
def _docker_image_available(image: str) -> bool:
    """Implements  docker image available.
...
def test_build_run_argv_minimal_defaults():
    """Validates behavior for build run argv minimal defaults.
    """
    argv = build_run_argv(
        binary="docker",
        image="ubuntu:24.04",
...
def test_build_run_argv_with_host_workdir_readonly():
    """Validates behavior for build run argv with host workdir readonly.
    """
    argv = build_run_argv(
        binary="docker",
        image="img",
...
def test_build_run_argv_with_host_workdir_writable():
    """Validates behavior for build run argv with host workdir writable.
    """
    argv = build_run_argv(
        binary="docker",
        image="img",
...
def test_build_run_argv_passes_extra_env_and_args():
    """Validates behavior for build run argv passes extra env and args.
    """
    argv = build_run_argv(
        binary="podman",
        image="alpine",
...
def test_build_exec_argv_interactive():
    """Validates behavior for build exec argv interactive.
    """
...
def test_dockershell_rejects_isolation_breaking_extra_run_args(extra):
    """Validates behavior for dockershell rejects isolation breaking extra run args.
    
    Args:
        extra: Description of extra.
    """
...
def test_dockershell_accepts_benign_extra_run_args():
    # Should not raise.
    """Validates behavior for dockershell accepts benign extra run args.
    """
    DockerShellTool(extra_run_args=("--label", "team=af", "--name-suffix", "x"))


def test_build_exec_argv_non_interactive_appends_dash_c():
    """Validates behavior for build exec argv non interactive appends dash c.
    """
...
def test_docker_shell_tool_validates_mode():
    """Validates behavior for docker shell tool validates mode.
    """
...
def test_docker_shell_tool_generates_unique_container_names():
    """Validates behavior for docker shell tool generates unique container names.
    """
    a = DockerShellTool()
    b = DockerShellTool()
...
def test_docker_shell_tool_implements_shell_executor_protocol():
    """Validates behavior for docker shell tool implements shell executor protocol.
    """
...
def test_as_function_carries_shell_kind():
    """Validates behavior for as function carries shell kind.
    """
...
async def test_start_and_close_are_noops_in_stateless_mode() -> None:
    """Validates behavior for start and close are noops in stateless mode.
    """
...
async def test_start_creates_and_reuses_persistent_session() -> None:
    """Validates behavior for start creates and reuses persistent session.
    """
...
async def test_close_terminates_session_and_container() -> None:
    """Validates behavior for close terminates session and container.
    """
    tool = DockerShellTool()
...
async def test_run_rejects_denied_commands() -> None:
    """Validates behavior for run rejects denied commands.
    """
    tool = DockerShellTool(
...
async def test_run_logs_audit_hook_failures_and_executes_persistent_command(
    ...
    """Validates behavior for run logs audit hook failures and executes persistent command.
    
    Args:
        caplog: Description of caplog.
    """
    def broken_hook(command: str) -> None:
        """Implements broken hook.
...
async def test_run_raises_if_start_did_not_create_persistent_session() -> None:
    """Validates behavior for run raises if start did not create persistent session.
    """
    tool = DockerShellTool()
...
async def test_run_dispatches_to_private_stateless_runner() -> None:
    """Validates behavior for run dispatches to private stateless runner.
    """
    tool = DockerShellTool(mode="stateless")
...
async def test_run_stateless_builds_expected_argv() -> None:
    """Validates behavior for run stateless builds expected argv.
    """
    tool = DockerShellTool(
        mode="stateless",
...
async def test_run_stateless_timeout_reaps_container_when_kill_fails() -> None:
    """Validates behavior for run stateless timeout reaps container when kill fails.
    """
    tool = DockerShellTool(mode="stateless")
...
async def test_run_stateless_timeout_handles_kill_and_reaper_timeouts() -> None:
    """Validates behavior for run stateless timeout handles kill and reaper timeouts.
    """
    tool = DockerShellTool(mode="stateless")
    command_proc = _FakeProcess(
        returncode=None,
...
async def test_start_container_success_logs_container_id(caplog: pytest.LogCaptureFixture) -> None:
    """Validates behavior for start container success logs container id.
...
async def test_start_container_raises_when_runtime_fails() -> None:
    """Validates behavior for start container raises when runtime fails.
    """
    tool = DockerShellTool()
...
async def test_stop_container_returns_after_first_success() -> None:
    """Validates behavior for stop container returns after first success.
    """
    tool = DockerShellTool()
...
async def test_stop_container_retries_when_first_attempt_fails() -> None:
    """Validates behavior for stop container retries when first attempt fails.
    """
    tool = DockerShellTool()
...
async def test_as_function_surfaces_command_errors() -> None:
    """Validates behavior for as function surfaces command errors.
    """
...
async def test_docker_persistent_session_preserves_state():
    """Validates behavior for docker persistent session preserves state.
    """
...
async def test_docker_stateless_each_command_isolated():
    """Validates behavior for docker stateless each command isolated.
    """
...
async def test_docker_no_network_by_default():
    """Validates behavior for docker no network by default.
    """
