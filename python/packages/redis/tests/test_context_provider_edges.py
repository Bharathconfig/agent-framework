...
def test_build_filter_from_dict_combines_multiple_tags(
    ...
    """Validates behavior for build filter from dict combines multiple tags.
    
    Args:
        patch_index: Description of patch_index.
    """
...
def test_schema_dict_includes_vector_configuration(
    ...
    """Validates behavior for schema dict includes vector configuration.
...
async def test_ensure_index_short_circuits_after_first_initialization(
    ...
    """Validates behavior for ensure index short circuits after first initialization.
...
async def test_ensure_index_validates_existing_schema_before_create(
    ...
    """Validates behavior for ensure index validates existing schema before create.
...
async def test_validate_schema_compatibility_raises_for_significant_mismatch(
    ...
    """Validates behavior for validate schema compatibility raises for significant mismatch.
...
async def test_add_requires_content_field(
    ...
    """Validates behavior for add requires content field.
...
async def test_add_vectorizes_documents_and_applies_defaults(
    ...
    """Validates behavior for add vectorizes documents and applies defaults.
...
async def test_redis_search_requires_non_empty_text(
    ...
    """Validates behavior for redis search requires non empty text.
...
async def test_redis_search_combines_explicit_filter_expression(
    ...
    """Validates behavior for redis search combines explicit filter expression.
...
async def test_search_all_collects_paginated_batches(
    ...
    """Validates behavior for search all collects paginated batches.
