...
def _build_app_with_agent(updates: list[AgentResponseUpdate], **kwargs: Any) -> FastAPI:
    """Implements  build app with agent.
...
def _service_session_runner(
    ...
    """Implements  service session runner.
...
async def test_service_session_full_idless_snapshot_sends_only_new_turn() -> None:
    """Validates behavior for service session full idless snapshot sends only new turn.
    """
...
async def test_service_session_matches_relayed_assistant_with_regenerated_id() -> None:
    """Validates behavior for service session matches relayed assistant with regenerated id.
    """
    runner, stub, store = _service_session_runner()
    await store.save(
        scope="service",
...
async def test_service_session_matches_relayed_tool_call_aliases() -> None:
    """Validates behavior for service session matches relayed tool call aliases.
    """
    runner, stub, store = _service_session_runner()
    await store.save(
        scope="service",
...
async def test_service_session_incremental_only_turn_preserves_snapshot_history() -> None:
    """Validates behavior for service session incremental only turn preserves snapshot history.
    """
    runner, stub, store = _service_session_runner()
    await _run_service_turn(
        runner,
...
async def test_service_session_stale_history_preserves_backend_assistant_message() -> None:
    """Validates behavior for service session stale history preserves backend assistant message.
    """
    runner, stub, store = _service_session_runner()
    await _run_service_turn(
        runner,
...
async def test_service_session_matches_truncated_history_overlap() -> None:
    """Validates behavior for service session matches truncated history overlap.
    """
    runner, stub, store = _service_session_runner()
    await store.save(
        scope="service",
...
async def test_service_session_does_not_drop_idless_repeat_of_first_user_turn() -> None:
    """Validates behavior for service session does not drop idless repeat of first user turn.
    """
    runner, stub, store = _service_session_runner()
    await store.save(
        scope="service",
...
async def test_service_session_does_not_drop_ambiguous_repeated_idless_user_turn() -> None:
    """Validates behavior for service session does not drop ambiguous repeated idless user turn.
    """
    runner, stub, store = _service_session_runner()
    await store.save(
        scope="service",
...
async def test_service_session_filters_forged_assistant_and_orphan_tool_suffix() -> None:
    """Validates behavior for service session filters forged assistant and orphan tool suffix.
    """
    runner, stub, store = _service_session_runner()
    await _run_service_turn(
        runner,
...
async def test_service_session_does_not_replay_balanced_tool_history() -> None:
    """Validates behavior for service session does not replay balanced tool history.
    """
    runner, stub, store = _service_session_runner(
        [
            AgentResponseUpdate(
...
async def test_service_session_keeps_incremental_trusted_tool_result() -> None:
    """Validates behavior for service session keeps incremental trusted tool result.
    """
    runner, stub, store = _service_session_runner()
    await store.save(
        scope="service",
...
async def test_service_session_legacy_approval_validates_against_authoritative_snapshot_history() -> None:
    """Validates behavior for service session legacy approval validates against authoritative snapshot history.
...
async def test_service_session_empty_generic_resume_invokes_agent_with_only_new_result() -> None:
    """Validates behavior for service session empty generic resume invokes agent with only new result.
    """
    runner, stub, store = _service_session_runner()
    await store.save(
        scope="service",
