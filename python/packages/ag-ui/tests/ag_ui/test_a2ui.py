...
class _RenderSub:
    ...
    def __init__(self, components=None, fragments=None):
        ...
        Args:
            ...
            fragments: Description of fragments.
        """
...
def _generate_envelope(kinds) -> Any:
    """Implements  generate envelope.
    
    Args:
        kinds: Description of kinds.
...
def test_context_slice_is_catalog_only():
    """Validates behavior for context slice is catalog only.
    """
    ctx = [
...
def test_inject_flag_sourced_from_forwarded_props_only():
    """Validates behavior for inject flag sourced from forwarded props only.
    """
...
def test_no_a2ui_context_yields_empty_slice():
    """Validates behavior for no a2ui context yields empty slice.
    """
...
def test_to_history_messages_extracts_tool_result_payload():
    """Validates behavior for to history messages extracts tool result payload.
    """
...
def test_a2ui_agent_prepends_catalog_system_message():
    """Validates behavior for a2ui agent prepends catalog system message.
    """
...
def test_a2ui_agent_passthrough_without_context():
    """Validates behavior for a2ui agent passthrough without context.
    """
...
def test_streaming_progressive_paint_balancing_and_envelope():
    """Validates behavior for streaming progressive paint balancing and envelope.
    """
...
def test_streaming_loop_ends_when_planner_stops_requesting():
    ...
    """Validates behavior for streaming loop ends when planner stops requesting.
    """
...
def test_streaming_closing_turn_withholds_generate_tool_at_round_cap():
    """Validates behavior for streaming closing turn withholds generate tool at round cap.
    ...
    class AlwaysGenerateInner:
        """Represents the AlwaysGenerateInner type and related behavior.
        """
        id = name = description = "planner"
...
def test_streaming_recovery_exhaustion():
    ...
    """Validates behavior for streaming recovery exhaustion.
    """
...
def test_streaming_retry_then_success():
    """Validates behavior for streaming retry then success.
    
    Returns:
        Description of the return value.
    """
    class RetrySub:
        """Represents the RetrySub type and related behavior.
        """
        def __init__(self):
            """Initializes a new instance.
...
def test_forwarded_schema_drives_validation_catalog():
    ...
    """Validates behavior for forwarded schema drives validation catalog.
    """
...
def test_recoverable_subagent_error_retries():
    """Validates behavior for recoverable subagent error retries.
    
    Returns:
        Description of the return value.
    """
    class RaisingSub:
        """Represents the RaisingSub type and related behavior.
        """
        def get_response(self, messages, *, stream=False, options=None):
            """Implements get response.
...
def test_programmer_error_is_rethrown():
    """Validates behavior for programmer error is rethrown.
    
    Returns:
        Description of the return value.
    """
    class TypeErrorSub:
        """Represents the TypeErrorSub type and related behavior.
        """
        def get_response(self, messages, *, stream=False, options=None):
            """Implements get response.
...
def test_mid_stream_death_balances_render_call():
    """Validates behavior for mid stream death balances render call.
    
    Returns:
        Description of the return value.
    """
    class PartialThenDieSub:
        """Represents the PartialThenDieSub type and related behavior.
        """
        def get_response(self, messages, *, stream=False, options=None):
            """Implements get response.
...
def test_non_streaming_tool_body_runs_recovery():
    """Validates behavior for non streaming tool body runs recovery.
    
    Returns:
        Description of the return value.
    """
    class NonStreamSub:
        """Represents the NonStreamSub type and related behavior.
        """
...
def test_enable_a2ui_returns_runner_wrapping_inner():
    """Validates behavior for enable a2ui returns runner wrapping inner.
    """
...
class _AgentWithClient:
    """Represents the _AgentWithClient type and related behavior.
    """
    id = name = description = "planner"
...
def test_plan_injection_off_without_flag():
    """Validates behavior for plan injection off without flag.
    """
...
def test_plan_injection_wraps_and_drops_render_tool():
    """Validates behavior for plan injection wraps and drops render tool.
    """
...
def test_plan_injection_string_flag_names_render_tool_to_drop():
    """Validates behavior for plan injection string flag names render tool to drop.
    """
    plan = plan_a2ui_injection(
        agent=_AgentWithClient(),
...
def test_plan_injection_user_prevails_when_generate_already_wired():
    """Validates behavior for plan injection user prevails when generate already wired.
    """
    plan = plan_a2ui_injection(
        agent=_AgentWithClient(),
        forwarded_props={"injectA2UITool": True},
...
def test_plan_injection_skips_already_wrapped_agent():
    """Validates behavior for plan injection skips already wrapped agent.
    """
...
def test_plan_injection_skips_when_no_client_inferable():
    """Validates behavior for plan injection skips when no client inferable.
    """
...
def test_plan_injection_runtime_false_beats_backend_opt_in():
    ...
    """Validates behavior for plan injection runtime false beats backend opt in.
    """
    assert (
        plan_a2ui_injection(
            agent=_AgentWithClient(),
...
def test_streaming_coalesces_fragmented_generate_call():
    """Validates behavior for streaming coalesces fragmented generate call.
    """
...
def test_streaming_render_selects_coalesced_args_not_doubled():
    ...
    """Validates behavior for streaming render selects coalesced args not doubled.
    """
...
def test_forwarded_list_catalog_normalized_not_crash():
    ...
    """Validates behavior for forwarded list catalog normalized not crash.
    """
...
def test_forwarded_catalog_id_binds_surface_when_unconfigured():
    ...
    """Validates behavior for forwarded catalog id binds surface when unconfigured.
    """
...
def test_planner_options_do_not_carry_context_slice():
    ...
    """Validates behavior for planner options do not carry context slice.
    
    Returns:
        Description of the return value.
    """
    class _RecordInner:
        """Represents the _RecordInner type and related behavior.
        """
        id = name = description = "planner"
...
def test_streaming_render_reassembles_deltas_only_provider():
    """Validates behavior for streaming render reassembles deltas only provider.
    """
...
def test_streaming_generate_reassembles_deltas_only_provider():
    """Validates behavior for streaming generate reassembles deltas only provider.
    """
...
def test_streaming_aclose_mid_render_does_not_raise_generator_exit():
    ...
    """Validates behavior for streaming aclose mid render does not raise generator exit.
...
def test_sanitize_unanswered_tool_calls_strips_dangling_a2ui_calls():
    ...
    """Validates behavior for sanitize unanswered tool calls strips dangling a2ui calls.
    """
...
def test_streaming_executes_ordinary_tool_called_with_generate():
    ...
    """Validates behavior for streaming executes ordinary tool called with generate.
...
def test_streaming_attributes_interleaved_fragments_by_index():
    """Validates behavior for streaming attributes interleaved fragments by index.
    ...
    class _CapturingAgent(A2UIAgent):
        """Represents the _CapturingAgent type and related behavior.
        """
        def _run_generate_streaming(self, call, conversation, state, box):
            """Implements  run generate streaming.
...
def test_a2ui_existing_tool_names_includes_agent_default_tools():
    ...
    """Validates behavior for a2ui existing tool names includes agent default tools.
    """
...
def test_mixed_batch_server_tool_runs_through_middleware_pipeline():
    ...
    """Validates behavior for mixed batch server tool runs through middleware pipeline.
    ...
    class _RecordingMiddleware(FunctionMiddleware):
        """Represents the _RecordingMiddleware type and related behavior.
        """
    ...
    class _ClientWithMiddleware:
        """Represents the _ClientWithMiddleware type and related behavior.
        """
    ...
    class _InnerWithClient(_SearchThenGenerateInner):
        """Represents the _InnerWithClient type and related behavior.
        """
...
def test_mixed_batch_middleware_failure_aborts_without_rendering():
    ...
    """Validates behavior for mixed batch middleware failure aborts without rendering.
    ...
    class _DenyMiddleware(FunctionMiddleware):
        """Represents the _DenyMiddleware type and related behavior.
        """
        async def process(self, context, call_next):
            """Implements process.
    ...
    class _ClientWithDenyMiddleware:
        """Represents the _ClientWithDenyMiddleware type and related behavior.
        """
    ...
    class _InnerWithDeny(_SearchThenGenerateInner):
        """Represents the _InnerWithDeny type and related behavior.
        """
...
def test_mixed_batch_tool_error_result_is_generic_not_leaking():
    ...
    """Validates behavior for mixed batch tool error result is generic not leaking.
    """
    from agent_framework import FunctionTool
...
def test_mixed_batch_client_tool_left_as_user_input_not_synthesized():
    ...
    """Validates behavior for mixed batch client tool left as user input not synthesized.
    """
    from agent_framework import FunctionTool
...
def test_mixed_batch_executes_agent_default_tool():
    ...
    """Validates behavior for mixed batch executes agent default tool.
    ...
    class _InnerWithDefaultTool(_SearchThenGenerateInner):
        ...
        """Represents the _InnerWithDefaultTool type and related behavior.
        """
...
async def test_bridge_client_tool_with_generate_surfaces_resumable_not_synthesized():
    ...
    """Validates behavior for bridge client tool with generate surfaces resumable not synthesized.
...
def test_a2ui_agent_delegates_run_loop_attrs_to_inner():
    ...
    """Validates behavior for a2ui agent delegates run loop attrs to inner.
    """
    class _Inner:
        """Represents the _Inner type and related behavior.
        """
...
def test_a2ui_agent_preserves_provider_state_authority_boundary():
    """Validates behavior for a2ui agent preserves provider state authority boundary.
    """
...
def test_a2ui_agent_conditionally_delegates_conversation_creation():
    """Validates behavior for a2ui agent conditionally delegates conversation creation.
...
def test_a2ui_agent_uses_per_request_context_over_constructor():
    ...
    """Validates behavior for a2ui agent uses per request context over constructor.
...
def test_facade_no_longer_advertises_removed_context_agent():
    """Validates behavior for facade no longer advertises removed context agent.
    """
...
class _FIConfigClient:
    ...
    def __init__(self, config, middleware=()):
        ...
        Args:
            ...
            middleware: Description of middleware.
        """
...
def test_mixed_batch_honors_max_function_calls_budget_across_rounds():
    ...
    """Validates behavior for mixed batch honors max function calls budget across rounds.
...
def test_generate_only_planner_honors_call_budget_then_narrates():
    ...
    """Validates behavior for generate only planner honors call budget then narrates.
    ...
    class _RepeatGenerateInner:
        """Represents the _RepeatGenerateInner type and related behavior.
        """
...
def test_generate_only_planner_honors_max_iterations():
    ...
    """Validates behavior for generate only planner honors max iterations.
...
def test_mixed_batch_skips_server_tool_when_invocation_disabled():
    ...
    """Validates behavior for mixed batch skips server tool when invocation disabled.
...
def test_mixed_batch_surfaces_approval_request_and_stops():
    ...
    """Validates behavior for mixed batch surfaces approval request and stops.
    ...
    class _ApprovalThenGenerateInner:
        """Represents the _ApprovalThenGenerateInner type and related behavior.
        """
...
def test_final_narration_turn_forces_tools_off():
    ...
    """Validates behavior for final narration turn forces tools off.
