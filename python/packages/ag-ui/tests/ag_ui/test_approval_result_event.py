...
def _weather_tool(executions: list[str]) -> FunctionTool:
    """Implements  weather tool.
    
    Args:
        executions: Description of executions.
...
async def _run_custom_approval(tool: FunctionTool) -> tuple[list[Any], StubAgent]:
    """Implements  run custom approval.
...
async def test_approved_call_emits_one_live_result_under_original_identity() -> None:
    """Validates behavior for approved call emits one live result under original identity.
    """
    executions: list[str] = []
...
async def test_rejected_call_does_not_execute_or_emit_live_result() -> None:
    """Validates behavior for rejected call does not execute or emit live result.
    """
    executions: list[str] = []
...
async def test_mixed_batch_preserves_approved_result_identity_and_order() -> None:
    """Validates behavior for mixed batch preserves approved result identity and order.
    """
    executions: list[str] = []
