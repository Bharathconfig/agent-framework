"""This module defines functionality for packages/hyperlight/tests/hyperlight/test_hyperlight_codeact.
...
def _hyperlight_integration_static_skip_reason() -> str | None:
    """Implements  hyperlight integration static skip reason.
...
def _hyperlight_integration_runtime_skip_reason() -> str | None:
    """Implements  hyperlight integration runtime skip reason.
...
def _skip_if_hyperlight_integration_runtime_disabled() -> None:
    """Implements  skip if hyperlight integration runtime disabled.
    """
...
def _close_sandbox(sandbox: Any) -> None:
    """Implements  close sandbox.
    
    Args:
        sandbox: Description of sandbox.
    """
...
def _close_execute_code_registry(execute_code: HyperlightExecuteCodeTool) -> None:
    """Implements  close execute code registry.
    
    Args:
        execute_code: Description of execute_code.
    """
...
def _close_provider_registry(provider: HyperlightCodeActProvider) -> None:
    """Implements  close provider registry.
...
def replacement_compute(a: int, b: int) -> int:
    """Implements replacement compute.
...
class _FakeResult:
    """Represents the _FakeResult type and related behavior.
    """
    success: bool
    stdout: str = ""
    stderr: str = ""
...
class _FakeSandbox:
    """Represents the _FakeSandbox type and related behavior.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            temp_output: Description of temp_output.
            backend: Description of backend.
            module: Description of module.
            module_path: Description of module_path.
            heap_size: Description of heap_size.
            stack_size: Description of stack_size.
        """
    ...
    def register_tool(self, name_or_tool: Any, callback: Any | None = None) -> None:
        """Implements register tool.
        
        Args:
            self: Description of self.
            name_or_tool: Description of name_or_tool.
            callback: Description of callback.
        """
        if callback is None:
    ...
    def restore(self, snapshot: Any) -> None:
        ...
        Args:
            self: Description of self.
            snapshot: Description of snapshot.
        """
        self.restore_calls.append(snapshot)

    def get_output_files(self) -> list[str]:
        """Implements get output files.
...
class _FakeRuntime:
    """Represents the _FakeRuntime type and related behavior.
    """
...
class _FakeSandboxWithoutOutputListing(_FakeSandbox):
    """Represents the _FakeSandboxWithoutOutputListing type and related behavior.
    """
    def get_output_files(self) -> list[str]:
        """Implements get output files.
...
class _FakeSandboxWithDelayedUnlistedOutput(_FakeSandboxWithoutOutputListing):
    """Represents the _FakeSandboxWithDelayedUnlistedOutput type and related behavior.
    """
...
class _FakeSessionContext:
    """Represents the _FakeSessionContext type and related behavior.
    """
    ...
    def extend_instructions(self, source_id: str, instructions: str) -> None:
        """Implements extend instructions.
...
def _extract_text_output(function_result: Content) -> str:
    """Implements  extract text output.
    
    Args:
        function_result: Description of function_result.
...
class _FakeCodeActChatClient(FunctionInvocationLayer[Any], BaseChatClient[Any]):
    """Represents the _FakeCodeActChatClient type and related behavior.
    """
    def __init__(self) -> None:
        """Initializes a new instance.
...
def test_execute_code_tool_updates_approval_with_managed_tools() -> None:
    """Validates behavior for execute code tool updates approval with managed tools.
    """
...
def test_execute_code_tool_replaces_tools_with_the_same_name() -> None:
    """Validates behavior for execute code tool replaces tools with the same name.
    """
    execute_code = HyperlightExecuteCodeTool(tools=[compute], _registry=_FakeRuntime())
...
async def test_execute_code_tool_parent_span_for_host_tools(
    ...
    """Validates behavior for execute code tool parent span for host tools.
    
    Args:
        span_exporter: Description of span_exporter.
        monkeypatch: Description of monkeypatch.
    """
...
def test_execute_code_tool_accepts_string_and_tuple_file_mounts_without_mode_flags(
    ...
    """Validates behavior for execute code tool accepts string and tuple file mounts without mode flags.
...
async def test_execute_code_tool_populates_input_dir_with_workspace_and_file_mounts(
    ...
    """Validates behavior for execute code tool populates input dir with workspace and file mounts.
...
def _create_junction_or_skip(*, link: Path, target: Path) -> None:
    """Implements  create junction or skip.
...
def test_resolve_contained_path_reports_runtime_resolve_failures(
    ...
    """Validates behavior for resolve contained path reports runtime resolve failures.
    ...
    def fail_candidate_resolve(self: Path, strict: bool = False) -> Path:
        """Implements fail candidate resolve.
        
        Args:
            self: Description of self.
            strict: Description of strict.
...
def test_populate_input_dir_rejects_symlink_to_file_outside_workspace(tmp_path: Path) -> None:
    """Validates behavior for populate input dir rejects symlink to file outside workspace.
...
def test_populate_input_dir_rejects_symlinked_directory_outside_workspace(tmp_path: Path) -> None:
    """Validates behavior for populate input dir rejects symlinked directory outside workspace.
...
def test_populate_input_dir_rejects_workspace_junction_outside_workspace(tmp_path: Path) -> None:
    """Validates behavior for populate input dir rejects workspace junction outside workspace.
...
def test_populate_input_dir_rejects_nested_workspace_junction(tmp_path: Path) -> None:
    """Validates behavior for populate input dir rejects nested workspace junction.
...
def test_populate_input_dir_rejects_file_mount_junction(tmp_path: Path) -> None:
    """Validates behavior for populate input dir rejects file mount junction.
...
class _SandboxWithListing:
    """Represents the _SandboxWithListing type and related behavior.
    """
    def __init__(self, output_files: list[str]) -> None:
        ...
        Args:
            self: Description of self.
            output_files: Description of output_files.
        """
...
def _decode_content_bytes(item: Content) -> bytes:
    """Implements  decode content bytes.
...
def test_clear_directory_removes_junction_without_deleting_target(tmp_path: Path) -> None:
    """Validates behavior for clear directory removes junction without deleting target.
...
def test_is_safe_output_file_rejects_runtime_resolve_failures(
    ...
    """Validates behavior for is safe output file rejects runtime resolve failures.
    ...
    def fail_report_resolve(self: Path, strict: bool = False) -> Path:
        """Implements fail report resolve.
...
def test_execute_code_tool_allowed_domains_use_structured_entries_and_replace_by_target() -> None:
    """Validates behavior for execute code tool allowed domains use structured entries and replace by target.
    """
    execute_code = HyperlightExecuteCodeTool(_registry=_FakeRuntime())
...
def test_execute_code_tool_normalizers_reject_invalid_inputs() -> None:
    """Validates behavior for execute code tool normalizers reject invalid inputs.
    """
...
def test_execute_code_tool_shape_guards_validate_pairs() -> None:
    """Validates behavior for execute code tool shape guards validate pairs.
    """
...
def test_instruction_builders_cover_mounted_paths_and_workspace_free_filesystem_state() -> None:
    """Validates behavior for instruction builders cover mounted paths and workspace free filesystem state.
    """
...
def test_execute_code_tool_description_contains_call_tool_guidance(tmp_path: Path) -> None:
    """Validates behavior for execute code tool description contains call tool guidance.
...
async def test_execute_code_tool_executes_with_structured_content(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for execute code tool executes with structured content.
...
async def test_execute_code_tool_collects_output_files_without_backend_listing(
    ...
    """Validates behavior for execute code tool collects output files without backend listing.
...
async def test_execute_code_tool_waits_for_unlisted_output_files_to_appear(
    ...
    """Validates behavior for execute code tool waits for unlisted output files to appear.
...
async def test_execute_code_tool_failure_returns_error_content(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for execute code tool failure returns error content.
...
async def test_execute_code_tool_retries_allowed_domains_with_urls_when_backend_rejects_host_targets(
    ...
    """Validates behavior for execute code tool retries allowed domains with urls when backend rejects host targets.
    ...
    class _FakeStrictNetworkSandbox:
        """Represents the _FakeStrictNetworkSandbox type and related behavior.
        """
...
def test_hyperlight_integration_runtime_skip_reason_reports_missing_hypervisor(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for hyperlight integration runtime skip reason reports missing hypervisor.
    ...
    class _FakeNoHypervisorSandbox:
        """Represents the _FakeNoHypervisorSandbox type and related behavior.
        """
        def __init__(
            self,
            *,
...
async def test_provider_injects_run_scoped_execute_code_tool() -> None:
    """Validates behavior for provider injects run scoped execute code tool.
    """
...
def test_provider_delegates_file_mounts_and_allowed_domains_to_internal_tool(tmp_path: Path) -> None:
    """Validates behavior for provider delegates file mounts and allowed domains to internal tool.
...
async def test_agent_runs_hyperlight_codeact_end_to_end_with_fake_sandbox(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for agent runs hyperlight codeact end to end with fake sandbox.
...
async def test_agent_runs_hyperlight_codeact_end_to_end_with_real_sandbox() -> None:
    """Validates behavior for agent runs hyperlight codeact end to end with real sandbox.
    """
...
async def test_provider_run_tool_writes_files_with_real_sandbox(tmp_path: Path) -> None:
    """Validates behavior for provider run tool writes files with real sandbox.
...
async def test_provider_run_tool_pings_bing_with_real_sandbox() -> None:
    """Validates behavior for provider run tool pings bing with real sandbox.
    """
    _skip_if_hyperlight_integration_runtime_disabled()
...
async def test_sandbox_runs_simple_code(restored_sandbox) -> None:
    """Validates behavior for sandbox runs simple code.
    
    Args:
        restored_sandbox: Description of restored_sandbox.
    """
...
async def test_sandbox_stdout_and_stderr_captured(restored_sandbox) -> None:
    """Validates behavior for sandbox stdout and stderr captured.
...
async def test_sandbox_code_failure_returns_nonzero_exit(restored_sandbox) -> None:
    """Validates behavior for sandbox code failure returns nonzero exit.
...
def test_sandbox_registry_close_shuts_down_workers(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for sandbox registry close shuts down workers.
...
def test_sandbox_registry_close_releases_per_entry_resources(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    class _ClosableFakeSandbox(_FakeSandbox):
        """Represents the _ClosableFakeSandbox type and related behavior.
        """
        def close(self) -> None:
            """Implements close.
...
async def test_make_sandbox_callback_propagates_exceptions() -> None:
    """Validates behavior for make sandbox callback propagates exceptions.
    """
    @tool
    def boom(x: int) -> int:
        """Always fails."""
...
class _OwnerThreadTrackedResult:
    ...
    def __init__(self, *, owner_thread: int, success: bool = True, stdout: str = "", stderr: str = "") -> None:
        ...
        Args:
            self: Description of self.
            owner_thread: Description of owner_thread.
...
def test_sandbox_is_finalized_on_owner_thread_after_registry_close(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class _OwnerDropFakeSandbox(_FakeSandbox):
        """Represents the _OwnerDropFakeSandbox type and related behavior.
        """
        def __init__(self, **kwargs: Any) -> None:
            """Initializes a new instance.
...
def test_worker_failure_does_not_leak_unsendable_via_exception_traceback(
    ...
    class _RaisingFakeSandbox(_FakeSandbox):
        """Represents the _RaisingFakeSandbox type and related behavior.
        """
        def __init__(self, **kwargs: Any) -> None:
            """Initializes a new instance.
