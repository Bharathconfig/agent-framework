"""This module defines functionality for packages/hyperlight/agent_framework_hyperlight/_execute_code_tool.
...
class _NormalizedFileMount:
    """Represents the _NormalizedFileMount type and related behavior.
    """
    host_path: Path
    mount_path: str
...
@dataclass(frozen=True, slots=True)
class _RunConfig:
    """Represents the _RunConfig type and related behavior.
    """
    backend: str
    module: str | None
...
class SandboxRuntime(Protocol):
    """Represents the SandboxRuntime type and related behavior.
    """
    def execute(self, *, config: _RunConfig, code: str) -> list[Content]: ...


class _NamedDirectory(Protocol):
    """Represents the _NamedDirectory type and related behavior.
    """
    name: str
...
def _collect_tools(*tool_groups: Any) -> list[FunctionTool]:
    """Implements  collect tools.
    
    Args:
        *tool_groups: Description of tool_groups.
...
def _resolve_execute_code_approval_mode(
    *,
    ...
    """Implements  resolve execute code approval mode.
    
    Args:
        base_approval_mode: Description of base_approval_mode.
...
def _resolve_existing_path(value: str | Path) -> Path:
    """Implements  resolve existing path.
...
def _resolve_workspace_root(value: str | Path | None) -> Path | None:
    """Implements  resolve workspace root.
...
def _is_relative_to_or_same(*, path: Path, root: Path) -> bool:
    """Implements  is relative to or same.
...
def _resolve_contained_path(*, path: Path, root: Path) -> Path:
    """Implements  resolve contained path.
...
def _inspect_stageable_input_path(*, path: Path, root: Path) -> os.stat_result:
    """Implements  inspect stageable input path.
...
def _is_resolved_under_root(*, path: Path, root: Path) -> bool:
    """Implements  is resolved under root.
...
def _normalize_file_mount_input(file_mount: FileMountInput) -> FileMount:
    """Implements  normalize file mount input.
    
    Args:
        file_mount: Description of file_mount.
...
def _normalize_http_method(method: str) -> str:
    """Implements  normalize http method.
...
def _normalize_http_methods(methods: str | Sequence[str] | None) -> tuple[str, ...] | None:
    """Implements  normalize http methods.
...
def _is_allowed_domain_pair(value: Any) -> TypeGuard[tuple[str, str | Sequence[str]]]:
    """Implements  is allowed domain pair.
...
def _normalize_allowed_domain_input(allowed_domain: AllowedDomainInput) -> AllowedDomain:
    """Implements  normalize allowed domain input.
    
    Args:
        allowed_domain: Description of allowed_domain.
...
def _allowed_domain_registration_targets(*, target: str, expand_missing_scheme: bool) -> tuple[str, ...]:
    """Implements  allowed domain registration targets.
    
    Args:
        target: Description of target.
        expand_missing_scheme: Description of expand_missing_scheme.
...
def _should_retry_allowed_domain_registration(
    *,
    ...
    """Implements  should retry allowed domain registration.
    
    Args:
        error: Description of error.
        allowed_domains: Description of allowed_domains.
...
def _normalize_mount_path(mount_path: str) -> str:
    """Implements  normalize mount path.
    
    Args:
        mount_path: Description of mount_path.
...
def _populate_input_dir(*, config: _RunConfig, input_root: Path) -> None:
    """Implements  populate input dir.
    
    Args:
        config: Description of config.
        input_root: Description of input_root.
    """
...
def _create_file_content(file_path: Path, *, relative_path: str) -> Content:
    """Implements  create file content.
    
    Args:
        file_path: Description of file_path.
        relative_path: Description of relative_path.
...
def _normalize_output_relative_path(*, output_file: object, root: Path) -> str | None:
    """Implements  normalize output relative path.
...
def _collect_output_relative_paths(*, sandbox: Any, root: Path) -> set[str]:
    """Implements  collect output relative paths.
...
def _parse_output_files(
    *,
    ...
    """Implements  parse output files.
    
    Args:
        sandbox: Description of sandbox.
        output_dir: Description of output_dir.
        expect_output_files: Description of expect_output_files.
...
def _build_execution_contents(
    *,
    ...
    """Implements  build execution contents.
...
def _make_sandbox_callback(tool_obj: FunctionTool) -> Callable[..., Any]:
    """Implements  make sandbox callback.
    
    Args:
        tool_obj: Description of tool_obj.
...
class _SandboxRegistry(SandboxRuntime):
    """Represents the _SandboxRegistry type and related behavior.
    """
    ...
    def _get_or_create_entry(self, config: _RunConfig) -> _SandboxEntry:
        """Implements  get or create entry.
...
class HyperlightExecuteCodeTool(FunctionTool):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            workspace_root: Description of workspace_root.
            file_mounts: Description of file_mounts.
            allowed_domains: Description of allowed_domains.
            backend: Description of backend.
            module: Description of module.
            module_path: Description of module_path.
            _registry: Description of _registry.
        """
        super().__init__(
            name="execute_code",
    ...
    def _refresh_approval_mode(self) -> None:
        """Implements  refresh approval mode.
