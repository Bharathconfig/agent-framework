...
class _ApprovalRequest:
    """Represents the _ApprovalRequest type and related behavior.
    """
    prompt: str
    request_id: str = ""
...
class ApprovalExecutor(_Executor, RequestInfoMixin):
    ...
    async def on_response(
        ...
        Args:
            ...
            approved: Description of approved.
            ctx: Description of ctx.
        """
...
def _build_passthrough_workflow() -> Workflow:
    """Implements  build passthrough workflow.
...
def _build_failing_workflow() -> Workflow:
    ...
    """Implements  build failing workflow.
...
def _build_approval_workflow() -> Workflow:
    """Implements  build approval workflow.
...
async def test_status_default_is_idle_before_first_run():
    """Validates behavior for status default is idle before first run.
    """
...
async def test_status_is_idle_after_successful_run():
    """Validates behavior for status is idle after successful run.
    """
    wf = _build_passthrough_workflow()
    await wf.run("hello")
    assert wf.status is WorkflowRunState.IDLE
...
async def test_status_is_failed_after_failure():
    """Validates behavior for status is failed after failure.
    """
...
async def test_status_idle_with_pending_requests_then_resolves_to_idle():
    """Validates behavior for status idle with pending requests then resolves to idle.
    """
