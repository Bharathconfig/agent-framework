"""This module defines functionality for packages/core/tests/workflow/test_workflow_states.
...
async def test_executor_failed_and_workflow_failed_events_streaming():
    """Validates behavior for executor failed and workflow failed events streaming.
    """
...
async def test_executor_failed_event_emitted_on_direct_execute():
    """Validates behavior for executor failed event emitted on direct execute.
    """
    failing = FailingExecutor(id="f")
...
async def test_idle_with_pending_requests_status_streaming():
    """Validates behavior for idle with pending requests status streaming.
    """
...
async def test_completed_status_streaming():
    """Validates behavior for completed status streaming.
    """
    c = Completer(id="c")
...
async def test_started_and_completed_event_origins():
    """Validates behavior for started and completed event origins.
    """
...
async def test_non_streaming_final_state_helpers():
    # Completed case
    """Validates behavior for non streaming final state helpers.
    """
    c = Completer(id="c")
...
async def test_run_includes_status_events_completed():
    """Validates behavior for run includes status events completed.
    """
    c = Completer(id="c2")
    wf = WorkflowBuilder(start_executor=c).build()
...
async def test_run_includes_status_events_idle_with_requests():
    """Validates behavior for run includes status events idle with requests.
    """
    simple_executor = SimpleExecutor(id="simple")
