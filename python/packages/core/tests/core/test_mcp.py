"""This module defines functionality for packages/core/tests/core/test_mcp.
...
def _reset_progressive_mcp_warning_state() -> None:
    """Implements  reset progressive mcp warning state.
    """
...
def _request_for_mcp_tool(tool: MCPStreamableHTTPTool, url: str = "http://example.com/mcp") -> Any:
    """Implements  request for mcp tool.
...
def test_build_prefixed_mcp_name_ignores_empty_normalized_prefix() -> None:
    """Validates behavior for build prefixed mcp name ignores empty normalized prefix.
    """
    assert _build_prefixed_mcp_name("search", "---") == "search"


def test_mcp_transport_subclasses_accept_tool_name_prefix() -> None:
    """Validates behavior for mcp transport subclasses accept tool name prefix.
    """
...
def test_mcp_tool_str_and_parse_prompt_result_rich_content() -> None:
    """Validates behavior for mcp tool str and parse prompt result rich content.
    """
...
def test_prepare_message_for_mcp():
    """Validates behavior for prepare message for mcp.
    """
    message = Message(
        role="user",
        contents=[
...
def test_prepare_message_for_mcp_skips_unsupported_content() -> None:
    """Validates behavior for prepare message for mcp skips unsupported content.
    """
...
async def test_local_mcp_server_context_manager():
    ...
    class TestServer(MCPTool):
        """Represents the TestServer type and related behavior.
        """
...
async def test_mcp_tool_is_error_propagates_through_function_middleware():
    ...
    class ErrorCheckMiddleware(FunctionMiddleware):
        """Represents the ErrorCheckMiddleware type and related behavior.
        """
...
async def test_local_mcp_server_prompt_execution():
    ...
    class TestMCPTool(MCPTool):
        """Represents the TestMCPTool type and related behavior.
        """
        async def connect(self):  # type: ignore[override]  # pyrefly: ignore[bad-override]  # ty: ignore[invalid-method-override]
            """Implements connect.
...
def test_mcp_tool_approval_mode_returns_none_for_unmatched_names() -> None:
    """Validates behavior for mcp tool approval mode returns none for unmatched names.
    """
    tool = MCPTool(  # type: ignore[abstract]
        name="test_tool",
        approval_mode={
...
def test_mcp_transport_subclasses_accept_progressive_disclosure_options() -> None:
    """Validates behavior for mcp transport subclasses accept progressive disclosure options.
    """
...
def test_mcp_progressive_disclosure_requires_loading_tools() -> None:
    """Validates behavior for mcp progressive disclosure requires loading tools.
    """
...
def test_mcp_progressive_disclosure_warns_on_construction() -> None:
    """Validates behavior for mcp progressive disclosure warns on construction.
    """
    _reset_progressive_mcp_warning_state()
...
def test_mcp_tool_base_constructor_preserves_positional_tool_name_prefix() -> None:
    """Validates behavior for mcp tool base constructor preserves positional tool name prefix.
    """
...
def _progressive_tool_list_page(*, tools: list[types.Tool] | None = None) -> types.ListToolsResult:
    """Implements  progressive tool list page.
...
async def _load_progressive_test_server(
    *,
    ...
    """Implements  load progressive test server.
    
    Args:
        allowed_tools: Description of allowed_tools.
        always_load: Description of always_load.
        tool_name_prefix: Description of tool_name_prefix.
        approval_mode: Description of approval_mode.
        tools: Description of tools.
...
async def test_mcp_progressive_disclosure_exposes_loaders_and_always_loaded_tools() -> None:
    """Validates behavior for mcp progressive disclosure exposes loaders and always loaded tools.
    """
...
async def test_mcp_progressive_disclosure_filters_always_loaded_loader_name_collisions() -> None:
    """Validates behavior for mcp progressive disclosure filters always loaded loader name collisions.
    """
...
async def test_mcp_progressive_disclosure_loader_names_honor_tool_name_prefix() -> None:
    """Validates behavior for mcp progressive disclosure loader names honor tool name prefix.
    """
...
async def test_mcp_progressive_list_mcp_tools_only_reports_allowed_tools() -> None:
    """Validates behavior for mcp progressive list mcp tools only reports allowed tools.
    """
...
async def test_mcp_progressive_list_mcp_tools_skips_loader_name_collisions() -> None:
    """Validates behavior for mcp progressive list mcp tools skips loader name collisions.
    """
    server = await _load_progressive_test_server(
        tools=[
            types.Tool(
...
async def test_mcp_progressive_list_mcp_tools_treats_empty_allowed_tools_as_no_tools() -> None:
    """Validates behavior for mcp progressive list mcp tools treats empty allowed tools as no tools.
    """
...
async def test_mcp_progressive_load_tool_adds_hidden_tool_to_live_tools() -> None:
    """Validates behavior for mcp progressive load tool adds hidden tool to live tools.
    """
    server = await _load_progressive_test_server()
...
async def test_mcp_progressive_load_tool_adds_multiple_hidden_tools_to_live_tools() -> None:
    """Validates behavior for mcp progressive load tool adds multiple hidden tools to live tools.
    """
    server = await _load_progressive_test_server()
    load_tool = server.functions[1]
    context = FunctionInvocationContext(
...
async def test_mcp_progressive_load_tool_reports_mixed_batch_results() -> None:
    """Validates behavior for mcp progressive load tool reports mixed batch results.
    """
...
async def test_mcp_progressive_unload_tool_removes_dynamically_loaded_tool() -> None:
    """Validates behavior for mcp progressive unload tool removes dynamically loaded tool.
    """
    server = await _load_progressive_test_server()
    load_tool = server.functions[1]
...
async def test_mcp_progressive_unload_tool_removes_multiple_dynamically_loaded_tools() -> None:
    """Validates behavior for mcp progressive unload tool removes multiple dynamically loaded tools.
    """
    server = await _load_progressive_test_server()
    load_tool = server.functions[1]
    unload_tool = server.functions[2]
...
async def test_mcp_progressive_unload_tool_rejects_always_loaded_tool() -> None:
    """Validates behavior for mcp progressive unload tool rejects always loaded tool.
    """
    server = await _load_progressive_test_server(always_load=["tool_one"])
    unload_tool = server.functions[2]
    context = FunctionInvocationContext(
...
async def test_mcp_progressive_unload_tool_reports_tool_not_loaded() -> None:
    """Validates behavior for mcp progressive unload tool reports tool not loaded.
    """
    server = await _load_progressive_test_server()
    unload_tool = server.functions[2]
    context = FunctionInvocationContext(
...
async def test_mcp_progressive_unload_tool_reports_mixed_batch_results() -> None:
    """Validates behavior for mcp progressive unload tool reports mixed batch results.
    """
    server = await _load_progressive_test_server(always_load=["tool_one"])
    load_tool = server.functions[1]
    unload_tool = server.functions[2]
...
async def test_mcp_progressive_load_and_unload_suppress_experimental_context_warnings() -> None:
    """Validates behavior for mcp progressive load and unload suppress experimental context warnings.
    """
    server = await _load_progressive_test_server()
    load_tool = server.functions[1]
    unload_tool = server.functions[2]
...
async def test_mcp_progressive_load_tool_rejects_filtered_tool() -> None:
    """Validates behavior for mcp progressive load tool rejects filtered tool.
    """
...
async def test_mcp_progressive_load_tool_rejects_missing_live_tool_list() -> None:
    """Validates behavior for mcp progressive load tool rejects missing live tool list.
    """
    server = await _load_progressive_test_server()
    load_tool = server.functions[1]
    context = FunctionInvocationContext(
...
async def test_mcp_progressive_load_tool_rejects_loader_name_collision() -> None:
    """Validates behavior for mcp progressive load tool rejects loader name collision.
    """
    server = await _load_progressive_test_server(
        tools=[
            types.Tool(
...
async def test_mcp_progressive_resolve_rejects_ambiguous_tool_name() -> None:
    """Validates behavior for mcp progressive resolve rejects ambiguous tool name.
    """
    def _tool() -> None:
        """Implements  tool.
        """
...
async def test_mcp_progressive_loaded_tool_preserves_remote_approval_mode() -> None:
    """Validates behavior for mcp progressive loaded tool preserves remote approval mode.
    """
...
async def test_mcp_progressive_loaded_http_tool_preserves_runtime_kwargs_for_headers() -> None:
    """Validates behavior for mcp progressive loaded http tool preserves runtime kwargs for headers.
...
async def test_mcp_connection_reset_integration():
    ...
    async with tool:
        ...
        async def call_tool_with_error(*args, **kwargs):
            """Implements call tool with error.
...
async def test_mcp_tool_message_handler_cancel_and_replace():
    ...
    async def blocking_load_tools():
        """Implements blocking load tools.
        """
        nonlocal call_count
        call_count += 1
        await release.wait()
...
def _make_sampling_response(text: str = "response", model: str = "test-model") -> Mock:
    """Implements  make sampling response.
...
async def test_mcp_tool_sampling_callback_callback_exception_denies():
    ...
    def boom(_params: object) -> bool:
        ...
        Args:
            _params: Description of _params.
        """
...
async def test_mcp_tool_logging_callback_logs_at_requested_level() -> None:
    """Validates behavior for mcp tool logging callback logs at requested level.
    """
    tool = MCPStdioTool(name="test_tool", command="python")
...
def test_should_propagate_cancelled_error_returns_false_for_non_cancelled_error():
    """Validates behavior for should propagate cancelled error returns false for non cancelled error.
    """
...
def test_should_propagate_cancelled_error_returns_false_when_no_current_task():
    """Validates behavior for should propagate cancelled error returns false when no current task.
    """
...
def test_should_propagate_cancelled_error_returns_true_when_task_is_cancelling():
    """Validates behavior for should propagate cancelled error returns true when task is cancelling.
    """
    mock_task = Mock()
...
def test_should_propagate_cancelled_error_returns_false_when_task_not_cancelling():
    """Validates behavior for should propagate cancelled error returns false when task not cancelling.
    """
    mock_task = Mock()
...
def test_describe_error_keeps_plain_exception_message():
    """Validates behavior for describe error keeps plain exception message.
    """
...
def test_describe_error_unwraps_single_member_exception_group():
    """Validates behavior for describe error unwraps single member exception group.
    """
    import builtins
...
def test_describe_error_unmasks_cancel_scope_via_context():
    """Validates behavior for describe error unmasks cancel scope via context.
    """
    real = RuntimeError("401 Client Error: Unauthorized")
...
def test_describe_error_keeps_bare_cancellation():
    """Validates behavior for describe error keeps bare cancellation.
    """
...
async def test_mcp_tool_get_prompt_reconnection_on_closed_resource_error():
    ...
    with patch.object(tool, "connect", new_callable=AsyncMock) as mock_connect:
        ...
        async def get_prompt_with_error(*args, **kwargs):
            """Implements get prompt with error.
...
async def test_mcp_tool_call_tool_requires_loaded_tools() -> None:
    """Validates behavior for mcp tool call tool requires loaded tools.
    """
...
async def test_mcp_tool_get_prompt_requires_loaded_prompts() -> None:
    """Validates behavior for mcp tool get prompt requires loaded prompts.
    """
...
async def test_mcp_tool_call_tool_raises_after_reconnection_still_fails() -> None:
    """Validates behavior for mcp tool call tool raises after reconnection still fails.
    """
    from anyio.streams.memory import ClosedResourceError
...
async def test_mcp_tool_get_prompt_raises_after_reconnection_still_fails() -> None:
    """Validates behavior for mcp tool get prompt raises after reconnection still fails.
    """
    from anyio.streams.memory import ClosedResourceError
...
async def test_mcp_tool_wraps_unexpected_call_tool_and_get_prompt_errors() -> None:
    """Validates behavior for mcp tool wraps unexpected call tool and get prompt errors.
    """
...
async def test_mcp_tool_aenter_wraps_unexpected_errors_and_closes() -> None:
    """Validates behavior for mcp tool aenter wraps unexpected errors and closes.
    """
    tool = MCPStdioTool(name="test_tool", command="python")
...
async def test_mcp_tool_close_cleans_up_in_original_task(caplog):
    ...
    class TaskBoundTransportContext:
        """Represents the TaskBoundTransportContext type and related behavior.
        """
...
async def test_mcp_tool_connect_from_lifecycle_owner_bypasses_request_lock() -> None:
    ...
    async def connect_from_owner_task() -> None:
        """Implements connect from owner task.
        """
...
async def test_mcp_tool_close_from_lifecycle_owner_bypasses_request_lock() -> None:
    ...
    async def close_from_owner_task() -> None:
        """Implements close from owner task.
        """
        tool._lifecycle_owner_task = asyncio.current_task()
        try:
            async with tool._lifecycle_request_lock:
...
async def test_connect_reinitializes_existing_session_and_loads_tools_and_prompts() -> None:
    """Validates behavior for connect reinitializes existing session and loads tools and prompts.
    """
    tool = MCPTool(name="test_tool", load_tools=True, load_prompts=True)  # type: ignore[abstract]
    tool.is_connected = True
    tool.session = Mock()
...
async def test_connect_skips_tools_and_prompts_when_server_does_not_advertise_capabilities() -> None:
    """Validates behavior for connect skips tools and prompts when server does not advertise capabilities.
    """
    tool = MCPTool(name="test_tool", load_tools=True, load_prompts=True)  # type: ignore[abstract]
    tool.is_connected = True
    tool.session = Mock()
...
async def test_connect_treats_missing_capabilities_as_unsupported() -> None:
    """Validates behavior for connect treats missing capabilities as unsupported.
    """
    tool = MCPTool(name="test_tool", load_tools=True, load_prompts=True)  # type: ignore[abstract]
    tool.is_connected = True
    tool.session = Mock()
...
async def test_connect_sets_logging_level_when_server_advertises_logging() -> None:
    """Validates behavior for connect sets logging level when server advertises logging.
    """
...
async def test_ensure_connected_skips_future_pings_when_ping_is_not_available() -> None:
    """Validates behavior for ensure connected skips future pings when ping is not available.
    """
    tool = MCPTool(name="test_tool")  # type: ignore[abstract]
    tool.session = Mock(
        send_ping=AsyncMock(
...
async def test_ensure_connected_reconnects_on_failed_ping() -> None:
    """Validates behavior for ensure connected reconnects on failed ping.
    """
    tool = MCPTool(name="test_tool")  # type: ignore[abstract]
...
async def test_ensure_connected_wraps_reconnect_failure() -> None:
    """Validates behavior for ensure connected wraps reconnect failure.
    """
    tool = MCPTool(name="test_tool")  # type: ignore[abstract]
    tool.session = Mock(send_ping=AsyncMock(side_effect=RuntimeError("closed")))
...
async def test_load_tools_reconnects_on_closed_resource_when_ping_is_unavailable() -> None:
    """Validates behavior for load tools reconnects on closed resource when ping is unavailable.
    """
...
async def test_load_prompts_reconnects_on_closed_resource_when_ping_is_unavailable() -> None:
    """Validates behavior for load prompts reconnects on closed resource when ping is unavailable.
    """
    from anyio import ClosedResourceError
...
async def test_mcp_tool_filters_framework_kwargs():
    ...
    class MockResponseFormat(BaseModel):
        """Represents the MockResponseFormat type and related behavior.
        """
        result: str
...
async def test_mcp_streamable_http_tool_header_provider_injects_headers():
    ...
    class _TestServer(MCPStreamableHTTPTool):
        """Represents the _TestServer type and related behavior.
        """
        async def connect(self):  # type: ignore[override]  # pyrefly: ignore[bad-override]  # ty: ignore[invalid-method-override]
            """Implements connect.
...
async def test_mcp_streamable_http_tool_header_provider_ambient_non_keyerror_propagates():
    ...
    class TokenRefreshError(RuntimeError):
        """Represents the TokenRefreshError type and related behavior.
        """
        pass

    def failing_provider(kw: dict[str, Any]) -> dict[str, str]:
        """Implements failing provider.
...
async def test_mcp_streamable_http_tool_removes_hook_without_mutating_active_hook_list():
    ...
    async def delayed_hook(_request: httpx.Request) -> None:
        """Implements delayed hook.
        
        Args:
            _request: Description of _request.
        """
        hook_started.set()
...
async def test_mcp_streamable_http_tool_header_provider_serializes_concurrent_calls():
    ...
    async def blocking_call_tool(tool_name, *, arguments=None, meta=None):
        """Implements blocking call tool.
        
        Args:
            tool_name: Description of tool_name.
            arguments: Description of arguments.
            meta: Description of meta.
...
def _make_task_snapshot(
    *,
    ...
    """Implements  make task snapshot.
    
    Args:
        task_id: Description of task_id.
        status: Description of status.
        status_message: Description of status_message.
        poll_interval_ms: Description of poll_interval_ms.
...
def _make_create_task_result(task_id: str = "task-1") -> types.CreateTaskResult:
    """Implements  make create task result.
...
def _make_payload(text: str = "done!", is_error: bool = False) -> types.GetTaskPayloadResult:
    """Implements  make payload.
    
    Args:
        text: Description of text.
        is_error: Description of is_error.
...
def _make_task_tool(
    ...
    *,
    ...
    """Implements  make task tool.
    
    Args:
        tool_name: Description of tool_name.
        task_support: Description of task_support.
        task_options: Description of task_options.
...
def _send_request_dispatcher(*responses_by_method: tuple[str, Any]) -> Any:
    ...
    async def _dispatch(request: Any, _result_type: Any, *_args: Any, **_kw: Any) -> Any:
        ...
        Args:
            request: Description of request.
            _result_type: Description of _result_type.
            *_args: Description of _args.
            **_kw: Description of _kw.
...
async def test_task_options_defaults_are_sane() -> None:
    """Validates behavior for task options defaults are sane.
    """
    from agent_framework import MCPTaskOptions
...
async def test_task_options_rejects_non_positive_default_ttl() -> None:
    """Validates behavior for task options rejects non positive default ttl.
    """
    from datetime import timedelta
...
async def test_load_tools_captures_task_support() -> None:
    """Validates behavior for load tools captures task support.
    """
...
async def test_call_tool_routes_required_through_task_lifecycle(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for call tool routes required through task lifecycle.
...
async def test_call_tool_as_task_default_ttl_propagates() -> None:
    """Validates behavior for call tool as task default ttl propagates.
    ...
    async def fake_send(request: Any, _result_type: Any, *_a: Any, **_kw: Any) -> Any:
        """Implements fake send.
        
        Args:
            request: Description of request.
            _result_type: Description of _result_type.
            *_a: Description of _a.
            **_kw: Description of _kw.
...
async def test_call_tool_as_task_sends_empty_task_metadata_when_ttl_none() -> None:
    ...
    """Validates behavior for call tool as task sends empty task metadata when ttl none.
...
async def test_call_tool_skips_task_path_for_optional_and_forbidden() -> None:
    """Validates behavior for call tool skips task path for optional and forbidden.
    """
...
async def test_call_tool_as_task_cancelled_status_raises() -> None:
    """Validates behavior for call tool as task cancelled status raises.
    """
    tool = _make_task_tool()
    tool.session.send_request = AsyncMock(  # type: ignore[method-assign, union-attr]  # ty: ignore[invalid-assignment]
        side_effect=_send_request_dispatcher(
...
async def test_call_tool_as_task_failed_status_raises() -> None:
    """Validates behavior for call tool as task failed status raises.
    """
    tool = _make_task_tool()
    tool.session.send_request = AsyncMock(  # type: ignore[method-assign, union-attr]  # ty: ignore[invalid-assignment]
        side_effect=_send_request_dispatcher(
...
async def test_call_tool_as_task_input_required_raises() -> None:
    """Validates behavior for call tool as task input required raises.
    """
    tool = _make_task_tool()
    tool.session.send_request = AsyncMock(  # type: ignore[method-assign, union-attr]  # ty: ignore[invalid-assignment]
        side_effect=_send_request_dispatcher(
...
async def test_call_tool_as_task_payload_iserror_raises() -> None:
    """Validates behavior for call tool as task payload iserror raises.
    """
    tool = _make_task_tool()
    tool.session.send_request = AsyncMock(  # type: ignore[method-assign, union-attr]  # ty: ignore[invalid-assignment]
        side_effect=_send_request_dispatcher(
...
async def test_call_tool_as_task_malformed_payload_raises() -> None:
    """Validates behavior for call tool as task malformed payload raises.
    """
    tool = _make_task_tool()
...
async def test_call_tool_as_task_method_not_found_falls_back() -> None:
    """Validates behavior for call tool as task method not found falls back.
    """
    tool = _make_task_tool()
    tool.session.send_request = AsyncMock(  # type: ignore[method-assign, union-attr]  # ty: ignore[invalid-assignment]
...
async def test_call_tool_as_task_invalid_params_falls_back() -> None:
    """Validates behavior for call tool as task invalid params falls back.
    """
    tool = _make_task_tool()
    tool.session.send_request = AsyncMock(  # type: ignore[method-assign, union-attr]  # ty: ignore[invalid-assignment]
...
async def test_call_tool_as_task_poll_interval_is_clamped(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for call tool as task poll interval is clamped.
...
async def test_call_tool_as_task_local_cancellation_fires_remote_cancel(
    ...
    """Validates behavior for call tool as task local cancellation fires remote cancel.
...
async def test_call_tool_as_task_cancellation_suppressed_when_disabled(
    ...
    """Validates behavior for call tool as task cancellation suppressed when disabled.
...
async def test_call_tool_as_task_reconnects_during_poll(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for call tool as task reconnects during poll.
...
async def test_call_tool_as_task_second_disconnect_raises_connection_lost(
    ...
    """Validates behavior for call tool as task second disconnect raises connection lost.
...
async def test_fetch_task_result_reconnects_during_fetch() -> None:
    """Validates behavior for fetch task result reconnects during fetch.
...
async def test_fetch_task_result_second_disconnect_raises_task_state_unknown_and_cancels() -> None:
    """Validates behavior for fetch task result second disconnect raises task state unknown and cancels.
...
async def test_call_tool_as_task_max_wait_exceeded_raises_and_cancels(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for call tool as task max wait exceeded raises and cancels.
...
async def test_call_tool_as_task_poll_transient_request_timeout_keeps_polling(
    ...
    """Validates behavior for call tool as task poll transient request timeout keeps polling.
...
async def test_call_tool_as_task_poll_hard_mcperror_cancels_and_raises(
    ...
    """Validates behavior for call tool as task poll hard mcperror cancels and raises.
...
async def test_try_cancel_task_logs_warning_on_timeout(
    ...
    """Validates behavior for try cancel task logs warning on timeout.
...
async def test_mcp_task_options_is_frozen() -> None:
    """Validates behavior for mcp task options is frozen.
    """
...
async def test_mcp_task_options_max_task_wait_rejects_non_positive() -> None:
    """Validates behavior for mcp task options max task wait rejects non positive.
    """
    from agent_framework import MCPTaskOptions
...
def test_normalize_additional_tool_argument_names_none() -> None:
    """Validates behavior for normalize additional tool argument names none.
    """
...
def test_normalize_additional_tool_argument_names_sequence() -> None:
    """Validates behavior for normalize additional tool argument names sequence.
    """
...
def test_normalize_additional_tool_argument_names_single_string() -> None:
    ...
    """Validates behavior for normalize additional tool argument names single string.
    """
...
def test_normalize_additional_tool_argument_names_mapping_with_global_key() -> None:
    """Validates behavior for normalize additional tool argument names mapping with global key.
    """
...
def test_normalize_additional_tool_argument_names_mapping_with_string_values() -> None:
    ...
    """Validates behavior for normalize additional tool argument names mapping with string values.
    """
    global_extras, per_tool = _normalize_additional_tool_argument_names({
        "*": "conversation_id",
        "tool_a": "custom",
...
def test_prepare_call_kwargs_strips_undeclared_arguments() -> None:
    """Validates behavior for prepare call kwargs strips undeclared arguments.
    """
    server = MCPTool(name="test_server")  # type: ignore[abstract]
...
def test_prepare_call_kwargs_global_extras_allowed() -> None:
    """Validates behavior for prepare call kwargs global extras allowed.
    """
...
def test_prepare_call_kwargs_per_tool_and_global_extras() -> None:
    """Validates behavior for prepare call kwargs per tool and global extras.
    """
    server = MCPTool(  # type: ignore[abstract]
        name="test_server",
...
def test_prepare_call_kwargs_denylist_guards_server_declared_names() -> None:
    ...
    """Validates behavior for prepare call kwargs denylist guards server declared names.
    """
    server = MCPTool(name="test_server", additional_tool_argument_names=["conversation_id"])  # type: ignore[abstract]
...
def test_prepare_call_kwargs_extras_override_denylist() -> None:
    ...
    """Validates behavior for prepare call kwargs extras override denylist.
    """
...
def test_prepare_call_kwargs_zero_arg_tool_passes_no_arguments() -> None:
    """Validates behavior for prepare call kwargs zero arg tool passes no arguments.
    """
    server = MCPTool(name="test_server")  # type: ignore[abstract]
...
def test_prepare_call_kwargs_unknown_tool_passes_only_global_extras() -> None:
    """Validates behavior for prepare call kwargs unknown tool passes only global extras.
    """
    server = MCPTool(name="test_server", additional_tool_argument_names=["conversation_id"])  # type: ignore[abstract]
...
def test_prepare_call_kwargs_extracts_meta() -> None:
    """Validates behavior for prepare call kwargs extracts meta.
    """
    server = MCPTool(name="test_server")  # type: ignore[abstract]
    server._tool_param_names_by_name = {"test_tool": {"param"}}
...
def test_prepare_call_kwargs_rejects_invalid_meta_key_names(key: str) -> None:
    """Validates behavior for prepare call kwargs rejects invalid meta key names.
