"""This module defines functionality for packages/core/tests/core/test_telemetry.
...
def test_agent_framework_user_agent_uses_core_distribution_version() -> None:
    """Validates behavior for agent framework user agent uses core distribution version.
    """
...
def test_mark_feature_used_supports_bit_127() -> None:
    """Validates behavior for mark feature used supports bit 127.
    """
    _reset_feature_mask()
    with (
        patch("agent_framework._telemetry.IS_TELEMETRY_ENABLED", True),
...
def test_mark_feature_used_rejects_out_of_range_bit(bit: int) -> None:
    """Validates behavior for mark feature used rejects out of range bit.
    
    Args:
        bit: Description of bit.
    """
    _reset_feature_mask()
    with (
        patch("agent_framework._telemetry.IS_TELEMETRY_ENABLED", True),
...
def test_feature_mask_env_var_disables_marking(disabled_value: str) -> None:
    """Validates behavior for feature mask env var disables marking.
    
    Args:
        disabled_value: Description of disabled_value.
    """
    _reset_feature_mask()
    with (
        patch("agent_framework._telemetry.IS_TELEMETRY_ENABLED", True),
...
def test_user_agent_env_var_disables_feature_mask() -> None:
    """Validates behavior for user agent env var disables feature mask.
    """
    _reset_feature_mask()
    with (
...
def test_apply_feature_token_adds_and_refreshes_live_mask() -> None:
    """Validates behavior for apply feature token adds and refreshes live mask.
    """
    _reset_feature_mask()
    with (
        patch("agent_framework._telemetry.IS_TELEMETRY_ENABLED", True),
...
def test_apply_feature_token_preserves_unrelated_comments() -> None:
    """Validates behavior for apply feature token preserves unrelated comments.
    """
    _reset_feature_mask()
    with (
        patch("agent_framework._telemetry.IS_TELEMETRY_ENABLED", True),
...
def test_remove_feature_token_strips_standalone_token() -> None:
    """Validates behavior for remove feature token strips standalone token.
    """
    assert remove_feature_token("(feat=v1.1)") == ""


def test_apply_feature_token_removes_stale_token_when_disabled() -> None:
    """Validates behavior for apply feature token removes stale token when disabled.
    """
    _reset_feature_mask()
    with (
        patch("agent_framework._telemetry.IS_TELEMETRY_ENABLED", True),
...
def test_mark_feature_used_is_thread_safe() -> None:
    """Validates behavior for mark feature used is thread safe.
    """
    _reset_feature_mask()
    with (
        patch("agent_framework._telemetry.IS_TELEMETRY_ENABLED", True),
...
def test_declared_feature_indexes_match_registry() -> None:
    """Validates behavior for declared feature indexes match registry.
    """
    registry_path = next(
        (
