"""This module defines functionality for packages/core/tests/core/test_harness_loop.
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            honor_response_format: Description of honor_response_format.
            service_mode: Description of service_mode.
        """
        super().__init__()
...
async def _resolve_should_continue_result(value: Any) -> Any:
    """Implements  resolve should continue result.
...
def test_rejects_non_positive_max_iterations(bad: int) -> None:
    """Validates behavior for rejects non positive max iterations.
    
    Args:
        bad: Description of bad.
    """
...
def test_judge_mode_default_max_iterations() -> None:
    """Validates behavior for judge mode default max iterations.
    """
...
def test_judge_mode_explicit_unbounded() -> None:
    """Validates behavior for judge mode explicit unbounded.
    """
...
def test_judge_mode_custom_max_iterations() -> None:
    """Validates behavior for judge mode custom max iterations.
    """
...
def test_constructor_configures_feedback_loop() -> None:
    """Validates behavior for constructor configures feedback loop.
    """
...
def test_constructor_sets_should_continue() -> None:
    """Validates behavior for constructor sets should continue.
    """
...
def test_with_judge_factory_builds_judge_condition() -> None:
    """Validates behavior for with judge factory builds judge condition.
    """
...
def test_with_judge_forwards_fresh_context() -> None:
    """Validates behavior for with judge forwards fresh context.
    """
...
async def test_loop_stops_at_max_iterations() -> None:
    """Validates behavior for loop stops at max iterations.
    """
...
async def test_should_continue_controls_iterations_and_receives_kwargs() -> None:
    """Validates behavior for should continue controls iterations and receives kwargs.
    ...
    def should_continue(*, iteration: int, last_result: AgentResponse, **kwargs: Any) -> bool:
        """Implements should continue.
        
        Args:
            iteration: Description of iteration.
            last_result: Description of last_result.
            **kwargs: Description of kwargs.
...
async def test_async_should_continue_is_awaited() -> None:
    """Validates behavior for async should continue is awaited.
...
async def test_default_next_message_nudge_is_used() -> None:
    """Validates behavior for default next message nudge is used.
    """
    client = RecordingChatClient()
...
async def test_custom_next_message_callable() -> None:
    """Validates behavior for custom next message callable.
...
async def test_next_message_returning_none_reuses_messages() -> None:
    """Validates behavior for next message returning none reuses messages.
    """
    client = RecordingChatClient()
...
async def test_non_streaming_returns_aggregated_transcript_by_default() -> None:
    """Validates behavior for non streaming returns aggregated transcript by default.
    """
...
async def test_non_streaming_return_final_only_returns_last_response() -> None:
    """Validates behavior for non streaming return final only returns last response.
    """
    client = RecordingChatClient(texts=["first answer", "second answer"])
    agent = Agent(
        client=client,
...
async def test_record_feedback_callable_captures_and_injects_progress() -> None:
    """Validates behavior for record feedback callable captures and injects progress.
...
async def test_feedback_fallback_records_response_text() -> None:
    """Validates behavior for feedback fallback records response text.
    """
    client = RecordingChatClient(texts=["first answer", "second answer"])
    agent = Agent(client=client, middleware=[AgentLoopMiddleware(always_continue, max_iterations=2)])
...
async def test_should_continue_feedback_flows_to_callables() -> None:
    """Validates behavior for should continue feedback flows to callables.
...
async def test_should_continue_plain_bool_yields_no_feedback() -> None:
    """Validates behavior for should continue plain bool yields no feedback.
...
async def test_inject_progress_false_exposes_kwarg_without_injecting() -> None:
    """Validates behavior for inject progress false exposes kwarg without injecting.
...
async def test_fresh_context_resets_to_original_task_plus_progress() -> None:
    """Validates behavior for fresh context resets to original task plus progress.
    """
    client = RecordingChatClient()
    agent = Agent(
        client=client,
...
def test_restore_session_resets_to_snapshot() -> None:
    """Validates behavior for restore session resets to snapshot.
    """
...
async def test_fresh_context_with_session_resets_history() -> None:
    ...
    """Validates behavior for fresh context with session resets history.
    """
...
class _Answer(BaseModel):
    """Represents the _Answer type and related behavior.
    """
    name: str
...
async def test_should_continue_marker_stops_loop_early() -> None:
    """Validates behavior for should continue marker stops loop early.
...
async def test_should_continue_callable_stops_loop_early() -> None:
    """Validates behavior for should continue callable stops loop early.
...
async def test_resolve_next_message_injects_full_log_without_session() -> None:
    """Validates behavior for resolve next message injects full log without session.
    """
...
async def test_resolve_next_message_injects_latest_entry_with_session() -> None:
    """Validates behavior for resolve next message injects latest entry with session.
    """
    mw = AgentLoopMiddleware(always_continue, max_iterations=5)
    loop_kwargs: dict[str, Any] = {
        "progress": ["e1", "e2"],
...
async def test_judge_stops_when_answered_on_first_pass() -> None:
    """Validates behavior for judge stops when answered on first pass.
    """
...
async def test_judge_continues_until_answered() -> None:
    """Validates behavior for judge continues until answered.
    """
    agent_client = RecordingChatClient()
...
async def test_judge_respects_default_max_iterations() -> None:
    """Validates behavior for judge respects default max iterations.
    """
    agent_client = RecordingChatClient()
...
async def test_judge_requests_structured_output() -> None:
    """Validates behavior for judge requests structured output.
    """
    agent_client = RecordingChatClient()
...
async def test_judge_uses_structured_value_to_stop() -> None:
    """Validates behavior for judge uses structured value to stop.
    """
    agent_client = RecordingChatClient()
...
async def test_judge_uses_structured_value_to_continue() -> None:
    """Validates behavior for judge uses structured value to continue.
    """
    agent_client = RecordingChatClient()
    judge_client = RecordingChatClient(
...
async def test_judge_feedback_is_returned_to_agent() -> None:
    """Validates behavior for judge feedback is returned to agent.
    """
    agent_client = RecordingChatClient()
    judge_client = RecordingChatClient(
        texts=[
...
async def test_judge_criteria_injects_agent_instruction_and_judge_criteria() -> None:
    """Validates behavior for judge criteria injects agent instruction and judge criteria.
    """
    agent_client = RecordingChatClient()
    judge_client = RecordingChatClient(texts=['{"answered": true}'], honor_response_format=True)
...
async def test_judge_custom_instructions_template_substitutes_criteria() -> None:
    """Validates behavior for judge custom instructions template substitutes criteria.
    """
    agent_client = RecordingChatClient()
    judge_client = RecordingChatClient(texts=['{"answered": true}'], honor_response_format=True)
...
async def test_judge_without_criteria_strips_placeholder() -> None:
    """Validates behavior for judge without criteria strips placeholder.
    """
    agent_client = RecordingChatClient()
    judge_client = RecordingChatClient(texts=['{"answered": true}'], honor_response_format=True)
...
async def test_additional_instructions_injected_as_system_message() -> None:
    """Validates behavior for additional instructions injected as system message.
    """
    client = RecordingChatClient()
    agent = Agent(
        client=client,
...
def test_background_tasks_running_helper_reflects_state() -> None:
    """Validates behavior for background tasks running helper reflects state.
    """
    ...
    class _DummyAgent:
        """Represents the _DummyAgent type and related behavior.
        """
        name = "worker"
...
def test_background_tasks_running_helper_requires_session_agent_and_provider() -> None:
    """Validates behavior for background tasks running helper requires session agent and provider.
    """
    from agent_framework import BackgroundAgentsProvider
...
def test_background_tasks_running_message_lists_running_tasks() -> None:
    """Validates behavior for background tasks running message lists running tasks.
    """
    from agent_framework import BackgroundAgentsProvider
...
def test_background_tasks_running_message_returns_none_when_idle() -> None:
    """Validates behavior for background tasks running message returns none when idle.
    """
    from agent_framework import BackgroundAgentsProvider
...
class _FakeHarnessAgent:
    ...
    def __init__(self, *providers: Any) -> None:
        ...
        Args:
            self: Description of self.
            *providers: Description of providers.
        """
...
async def test_todos_remaining_reflects_store_state() -> None:
    """Validates behavior for todos remaining reflects store state.
    """
...
async def test_todos_remaining_requires_session_agent_and_provider() -> None:
    """Validates behavior for todos remaining requires session agent and provider.
    """
    provider = TodoProvider()
    session = AgentSession()
    await _save_todos(provider, session, [TodoItem(id=1, title="open", is_complete=False)])
...
async def test_todos_remaining_mode_gating() -> None:
    """Validates behavior for todos remaining mode gating.
    """
    provider = TodoProvider()
...
async def test_todos_remaining_modes_none_ignores_mode() -> None:
    """Validates behavior for todos remaining modes none ignores mode.
    """
    provider = TodoProvider()
    mode_provider = AgentModeProvider()
    session = AgentSession()
...
def test_todos_remaining_rejects_empty_modes() -> None:
    """Validates behavior for todos remaining rejects empty modes.
    """
...
async def test_todos_remaining_message_lists_open_items() -> None:
    """Validates behavior for todos remaining message lists open items.
    """
    provider = TodoProvider()
    session = AgentSession()
    agent = _FakeHarnessAgent(provider)
...
async def test_todos_remaining_message_returns_none_when_unavailable() -> None:
    """Validates behavior for todos remaining message returns none when unavailable.
    """
    provider = TodoProvider()
    session = AgentSession()
    agent = _FakeHarnessAgent(provider)
...
async def test_streaming_reyields_updates_and_final_is_last_iteration() -> None:
    """Validates behavior for streaming reyields updates and final is last iteration.
...
async def test_streaming_injects_nudge_messages_as_user_updates() -> None:
    """Validates behavior for streaming injects nudge messages as user updates.
    """
    client = RecordingChatClient(texts=["first", "second"])
    agent = Agent(client=client, middleware=[AgentLoopMiddleware(always_continue, max_iterations=2)])
...
async def test_streaming_stops_at_max_iterations() -> None:
    """Validates behavior for streaming stops at max iterations.
    """
    client = RecordingChatClient()
    agent = Agent(client=client, middleware=[AgentLoopMiddleware(always_continue, max_iterations=2)])
...
async def test_streaming_should_continue_marker_stops_and_injects_progress() -> None:
    """Validates behavior for streaming should continue marker stops and injects progress.
...
async def test_streaming_middleware_termination_stops_cleanly() -> None:
    """Validates behavior for streaming middleware termination stops cleanly.
    """
    client = RecordingChatClient(texts=["only"])

    class TerminateOnSecond(AgentMiddleware):
        """Represents the TerminateOnSecond type and related behavior.
        """
...
def test_has_pending_approval_request_detects_request() -> None:
    """Validates behavior for has pending approval request detects request.
    """
...
def test_has_pending_approval_request_false_for_plain_response() -> None:
    """Validates behavior for has pending approval request false for plain response.
    """
...
async def test_non_streaming_escape_hatch_stops_on_pending_approval() -> None:
    """Validates behavior for non streaming escape hatch stops on pending approval.
...
async def test_streaming_escape_hatch_stops_on_pending_approval() -> None:
    """Validates behavior for streaming escape hatch stops on pending approval.
...
class _TurnScopedRecordingProvider(ContextProvider):
    """Represents the _TurnScopedRecordingProvider type and related behavior.
    """
...
class _RunScopedRecordingProvider(ContextProvider):
    """Represents the _RunScopedRecordingProvider type and related behavior.
    """
    def __init__(self) -> None:
        """Initializes a new instance.
...
class _StrictTransportChatClient(RecordingChatClient):
    """Represents the _StrictTransportChatClient type and related behavior.
    """
    def __init__(self) -> None:
        """Initializes a new instance.
    ...
    def _strict_transport(self, *, tool_choice: Any, _provider_option: str) -> None:
        """Implements  strict transport.
        
        Args:
            self: Description of self.
            tool_choice: Description of tool_choice.
            _provider_option: Description of _provider_option.
        """
...
async def test_loop_marker_is_not_forwarded_to_provider_transport(stream: bool) -> None:
    """Validates behavior for loop marker is not forwarded to provider transport.
...
async def test_turn_scoped_after_run_fires_once_per_loop() -> None:
    """Validates behavior for turn scoped after run fires once per loop.
    """
    client = RecordingChatClient()
    turn_scoped = _TurnScopedRecordingProvider()
...
async def test_turn_scoped_after_run_fires_once_per_loop_streaming() -> None:
    """Validates behavior for turn scoped after run fires once per loop streaming.
    """
    client = RecordingChatClient()
    turn_scoped = _TurnScopedRecordingProvider()
    run_scoped = _RunScopedRecordingProvider()
...
async def test_turn_scoped_after_run_fires_normally_without_loop() -> None:
    """Validates behavior for turn scoped after run fires normally without loop.
    """
    client = RecordingChatClient()
    turn_scoped = _TurnScopedRecordingProvider()
...
def test_compaction_provider_defers_to_turn_boundary() -> None:
    """Validates behavior for compaction provider defers to turn boundary.
    """
...
class _OptionsRecordingProvider(ContextProvider):
    """Represents the _OptionsRecordingProvider type and related behavior.
    """
    after_run_once_per_turn = True
...
async def test_turn_scoped_after_run_sees_run_options() -> None:
    """Validates behavior for turn scoped after run sees run options.
    """
    client = RecordingChatClient()
...
async def test_nested_agent_run_inside_loop_iteration_is_not_suppressed() -> None:
    """Validates behavior for nested agent run inside loop iteration is not suppressed.
...
async def test_nested_same_agent_run_with_separate_session_is_not_suppressed() -> None:
    """Validates behavior for nested same agent run with separate session is not suppressed.
...
async def test_concurrent_same_agent_run_during_stream_pause_is_not_suppressed() -> None:
    """Validates behavior for concurrent same agent run during stream pause is not suppressed.
    """
    turn_scoped = _TurnScopedRecordingProvider()
    agent = Agent(
        client=RecordingChatClient(),
