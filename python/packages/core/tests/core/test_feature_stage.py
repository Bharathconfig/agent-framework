"""This module defines functionality for packages/core/tests/core/test_feature_stage.
...
class AlternateExperimentalFeature(str, Enum):
    """Represents the AlternateExperimentalFeature type and related behavior.
    """
...
class InvalidStageFeature(str, Enum):
    """Represents the InvalidStageFeature type and related behavior.
    """
    LOWERCASE = "skills"


class NonStringFeature(Enum):
    """Represents the NonStringFeature type and related behavior.
    """
    INTEGER = 1


class HelperReleaseCandidateFeature(str, Enum):
    """Represents the HelperReleaseCandidateFeature type and related behavior.
    """
...
def clear_feature_warning_state() -> Generator[None]:  # type: ignore[misc]  # pyrefly: ignore[bad-return]
    """Implements clear feature warning state.
    """
...
def test_feature_enums_are_exposed_from_root() -> None:
    """Validates behavior for feature enums are exposed from root.
    """
...
def test_experimental_decorator_accepts_feature_enum() -> None:
    """Validates behavior for experimental decorator accepts feature enum.
    """
...
def test_experimental_function_warns_on_call_and_not_on_definition() -> None:
    """Validates behavior for experimental function warns on call and not on definition.
...
def test_experimental_class_warns_on_instantiation_and_not_on_definition() -> None:
    """Validates behavior for experimental class warns on instantiation and not on definition.
    """
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
...
def test_experimental_abc_subclass_warning_points_at_user_file() -> None:
    ...
    class ExperimentalABC(ABC):
        """Represents the ExperimentalABC type and related behavior.
        """
        @abstractmethod
        def do(self) -> int: ...
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        class Concrete(ExperimentalABC):
            """Represents the Concrete type and related behavior.
            """
            def do(self) -> int:
                """Implements do.
...
def test_experimental_internal_framework_subclass_does_not_warn() -> None:
    """Validates behavior for experimental internal framework subclass does not warn.
    """
    @experimental(feature_id=AlternateExperimentalFeature.EXPERIMENTAL_FEATURE)  # type: ignore[arg-type]  # ty: ignore[invalid-argument-type]
    class ExperimentalBase:
        """Represents the ExperimentalBase type and related behavior.
        """
        pass
...
def test_experimental_runtime_checkable_protocol_keeps_protocol_runtime_checks() -> None:
    """Validates behavior for experimental runtime checkable protocol keeps protocol runtime checks.
...
def test_experimental_warning_is_emitted_once_per_feature() -> None:
    """Validates behavior for experimental warning is emitted once per feature.
    """
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        ...
        @experimental(feature_id=AlternateExperimentalFeature.SHARED_FEATURE)  # type: ignore[arg-type]  # ty: ignore[invalid-argument-type]
        class Second:
            """Represents the Second type and related behavior.
            """
            pass
...
def test_release_candidate_internal_helper_adds_metadata_without_runtime_warning() -> None:
    """Validates behavior for release candidate internal helper adds metadata without runtime warning.
    """
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
...
def test_experimental_property_warns_on_access_and_not_on_definition() -> None:
    """Validates behavior for experimental property warns on access and not on definition.
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        class Example:
            """Represents the Example type and related behavior.
            """
            @property
            @experimental(feature_id=AlternateExperimentalFeature.EXPERIMENTAL_FEATURE)  # type: ignore[arg-type]  # ty: ignore[invalid-argument-type]
            def value(self) -> int:
...
def test_experimental_staticmethod_warns_when_decorator_wraps_descriptor() -> None:
    """Validates behavior for experimental staticmethod warns when decorator wraps descriptor.
...
def test_experimental_classmethod_warns_when_decorator_wraps_descriptor() -> None:
    """Validates behavior for experimental classmethod warns when decorator wraps descriptor.
...
def test_feature_id_allows_lowercase_values() -> None:
    """Validates behavior for feature id allows lowercase values.
    """
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
...
def test_experimental_decorator_allows_string_feature_id_at_runtime() -> None:
    """Validates behavior for experimental decorator allows string feature id at runtime.
    """
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
...
def test_experimental_decorator_allows_other_enum_values_at_runtime() -> None:
    """Validates behavior for experimental decorator allows other enum values at runtime.
    """
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
...
def test_release_candidate_decorator_allows_string_feature_id_at_runtime() -> None:
    """Validates behavior for release candidate decorator allows string feature id at runtime.
    """
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
...
def test_feature_id_stringifies_non_string_enum_values() -> None:
    """Validates behavior for feature id stringifies non string enum values.
    """
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
