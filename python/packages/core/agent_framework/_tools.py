"""This module defines functionality for packages/core/agent_framework/_tools.
...
def _raise_duplicate_tool_name(tool_name: str, duplicate_error_message: str | None = None) -> None:
    """Implements  raise duplicate tool name.
    
    Args:
        tool_name: Description of tool_name.
        duplicate_error_message: Description of duplicate_error_message.
    """
...
def _append_unique_tools(
    ...
    *,
    ...
    """Implements  append unique tools.
    
    Args:
        existing_tools: Description of existing_tools.
        new_tools: Description of new_tools.
        duplicate_error_message: Description of duplicate_error_message.
...
def _ensure_unique_tool_names(
    ...
    *,
    ...
    """Implements  ensure unique tool names.
...
def normalize_function_invocation_configuration(
    ...
    """Implements normalize function invocation configuration.
...
def _function_execution_error_result(
    ...
    """Implements  function execution error result.
    
    Args:
        function_call: Description of function_call.
        tool_name: Description of tool_name.
...
async def _auto_invoke_function(
    ...
    *,
    ...
    async def final_function_handler(context_obj: Any) -> Any:
        """Implements final function handler.
        
        Args:
            context_obj: Description of context_obj.
...
def _is_actionable_function_call(content: Content) -> bool:
    """Implements  is actionable function call.
...
def _underlying_function_call(content: Content) -> Content:
    """Implements  underlying function call.
...
async def _execute_single_function_call(
    ...
    *,
    ...
    """Implements  execute single function call.
    
    Args:
        function_call: Description of function_call.
        custom_args: Description of custom_args.
        config: Description of config.
        tool_map: Description of tool_map.
        invocation_session: Description of invocation_session.
        middleware_pipeline: Description of middleware_pipeline.
        live_tools: Description of live_tools.
...
async def _execute_function_calls(
    *,
    ...
    """Implements  execute function calls.
    
    Args:
        custom_args: Description of custom_args.
        function_calls: Description of function_calls.
...
def _clear_internal_conversation_id(response: ChatResponse[Any]) -> ChatResponse[Any]:
    """Implements  clear internal conversation id.
...
def _is_unexecutable_local_tool_content(content: Content) -> bool:
    """Implements  is unexecutable local tool content.
...
def _response_has_visible_content(response: ChatResponse[Any]) -> bool:
    """Implements  response has visible content.
...
def _response_has_hosted_tool_approval(response: ChatResponse[Any]) -> bool:
    """Implements  response has hosted tool approval.
...
def _drop_unexecutable_tool_contents_from_response(response: ChatResponse[Any]) -> None:
    """Implements  drop unexecutable tool contents from response.
...
def _ensure_function_invocation_limit_fallback_response(response: ChatResponse[Any]) -> bool:
    """Implements  ensure function invocation limit fallback response.
...
def _function_invocation_limit_fallback_update() -> ChatResponseUpdate:
    """Implements  function invocation limit fallback update.
...
def _update_has_meaningful_metadata(update: ChatResponseUpdate) -> bool:
    """Implements  update has meaningful metadata.
...
def _drop_unexecutable_tool_contents_from_update(update: ChatResponseUpdate) -> ChatResponseUpdate | None:
    """Implements  drop unexecutable tool contents from update.
...
def _collect_unanswered_approval_requests(messages: Sequence[Message]) -> list[Content]:
    """Implements  collect unanswered approval requests.
...
def _remove_unanswered_approval_batches_from_model_input(messages: list[Message]) -> None:
    """Implements  remove unanswered approval batches from model input.
...
class _ApprovalCallOccurrence:
    """Represents the _ApprovalCallOccurrence type and related behavior.
    """
    function_call: Content
...
def _replace_approval_contents_with_results(
    ...
    def find_open_occurrence(call_id: str, *, require_unbound: bool = False) -> _ApprovalCallOccurrence | None:
        """Implements find open occurrence.
        
        Args:
            call_id: Description of call_id.
            require_unbound: Description of require_unbound.
    ...
    def find_approval_occurrence(approval_id: str) -> _ApprovalCallOccurrence | None:
        """Implements find approval occurrence.
...
def _extract_function_calls(response: ChatResponse) -> list[Content]:
    """Implements  extract function calls.
...
def _prepend_function_call_messages(response: ChatResponse, function_call_messages: list[Message]) -> None:
    """Implements  prepend function call messages.
    
    Args:
        response: Description of response.
        function_call_messages: Description of function_call_messages.
    """
    response.messages[:0] = function_call_messages


def _copy_messages_for_function_invocation(messages: Any) -> list[Message]:
    """Implements  copy messages for function invocation.
...
def _function_call_limit_reached(total_function_calls: int, max_function_calls: int | None) -> bool:
    """Implements  function call limit reached.
    
    Args:
        total_function_calls: Description of total_function_calls.
        max_function_calls: Description of max_function_calls.
...
def _update_consecutive_error_count(
    ...
    *,
    ...
    """Implements  update consecutive error count.
    
    Args:
        errors_in_a_row: Description of errors_in_a_row.
        had_errors: Description of had_errors.
        max_errors: Description of max_errors.
...
def _disable_tools_at_function_call_limit(
    ...
    """Implements  disable tools at function call limit.
...
def _record_function_calls(
    ...
    """Implements  record function calls.
    
    Args:
        budget_state: Description of budget_state.
        total_function_calls: Description of total_function_calls.
        function_call_count: Description of function_call_count.
...
def _reset_required_tool_choice(options: dict[str, Any]) -> None:
    """Implements  reset required tool choice.
...
def _prepare_messages_for_next_iteration(prepared_messages: list[Message], response: ChatResponse[Any]) -> None:
    """Implements  prepare messages for next iteration.
    
    Args:
        prepared_messages: Description of prepared_messages.
        response: Description of response.
    """
...
def _messages_and_updates_for_terminal_contents(
    ...
    """Implements  messages and updates for terminal contents.
...
class FunctionInvocationLayer(Generic[OptionsCoT]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            function_invocation_configuration: Description of function_invocation_configuration.
    ...
    def _get_function_middleware_pipeline(
        ...
        """Implements  get function middleware pipeline.
        
        Args:
            self: Description of self.
            runtime_middleware: Description of runtime_middleware.
    ...
    async def _get_response_with_function_invocation(
        ...
        *,
        ...
        async def settle_approval_replay_calls(function_calls: Sequence[Content]) -> None:
            """Implements settle approval replay calls.
    ...
    def get_response(
        ...
        *,
        ...
        Args:
            ...
            options: Description of options.
            middleware: Description of middleware.
            compaction_strategy: Description of compaction_strategy.
            tokenizer: Description of tokenizer.
