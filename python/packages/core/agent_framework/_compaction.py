"""This module defines functionality for packages/core/agent_framework/_compaction.
...
def _is_tool_call_assistant(message: Message) -> bool:
    """Implements  is tool call assistant.
...
def _is_reasoning_only_assistant(message: Message) -> bool:
    """Implements  is reasoning only assistant.
...
def _unambiguous_function_call_result_pairs(messages: Sequence[Message]) -> list[tuple[int, int]]:
    """Implements  unambiguous function call result pairs.
...
def _ensure_message_ids(
    ...
    """Implements  ensure message ids.
    
    Args:
        messages: Description of messages.
        id_offset: Description of id_offset.
        reserved_ids: Description of reserved_ids.
    """
...
def _group_id_for(message: Message, group_index: int) -> str:
    """Implements  group id for.
    
    Args:
        message: Description of message.
        group_index: Description of group_index.
...
def _link_function_call_result_spans(messages: Sequence[Message], spans: list[dict[str, Any]]) -> None:
    ...
    def find(span_index: int) -> int:
        ...
        Args:
            span_index: Description of span_index.
...
def _read_group_annotation(message: Message) -> dict[str, Any] | None:
    """Implements  read group annotation.
...
def _read_group_annotation_raw(message: Message) -> dict[str, Any] | None:
    """Implements  read group annotation raw.
...
def _set_group_summarized_by_summary_id(message: Message, summary_id: str) -> None:
    """Implements  set group summarized by summary id.
    
    Args:
        message: Description of message.
        summary_id: Description of summary_id.
    """
...
def _reconcile_compaction_summaries(  # pyright: ignore[reportUnusedFunction]
    ...
    def source_dependencies(summary_id: str) -> set[str]:
        """Implements source dependencies.
...
def _write_group_annotation(
    ...
    *,
    ...
    """Implements  write group annotation.
    ...
    Args:
        ...
        has_reasoning: Description of has_reasoning.
    """
...
def _ordered_group_ids_from_annotations(messages: Sequence[Message]) -> list[str]:
    """Implements  ordered group ids from annotations.
...
def _first_untokenized_index(messages: Sequence[Message]) -> int | None:
    """Implements  first untokenized index.
...
def _first_annotation_gaps(
    ...
    *,
    ...
    """Implements  first annotation gaps.
    
    Args:
        messages: Description of messages.
        include_tokens: Description of include_tokens.
...
def _function_pair_reannotation_start(messages: Sequence[Message], start_index: int) -> int:
    """Implements  function pair reannotation start.
    
    Args:
        messages: Description of messages.
        start_index: Description of start_index.
...
def included_token_count(messages: list[Message]) -> int:
    """Implements included token count.
...
def set_excluded(message: Message, *, excluded: bool, reason: str | None = None) -> bool:
    """Implements set excluded.
    
    Args:
        message: Description of message.
        excluded: Description of excluded.
...
def exclude_group_ids(messages: list[Message], group_ids: set[str], *, reason: str) -> bool:
    """Implements exclude group ids.
    
    Args:
        messages: Description of messages.
        group_ids: Description of group_ids.
        reason: Description of reason.
...
def project_included_messages(messages: list[Message]) -> list[Message]:
    """Implements project included messages.
...
def _group_messages_by_id(messages: list[Message]) -> dict[str, list[Message]]:
    """Implements  group messages by id.
...
def _group_start_indices(messages: list[Message]) -> dict[str, int]:
    """Implements  group start indices.
...
def _included_group_ids(messages: list[Message], ordered_group_ids: list[str]) -> list[str]:
    """Implements  included group ids.
    
    Args:
        messages: Description of messages.
        ordered_group_ids: Description of ordered_group_ids.
...
def _count_included_messages(messages: list[Message]) -> int:
    """Implements  count included messages.
...
def _count_included_tokens(messages: list[Message]) -> int:
    """Implements  count included tokens.
...
class ToolResultCompactionStrategy:
    ...
    def _summary_text(cls, tool_results: list[str]) -> str:
        """Implements  summary text.
        
        Args:
            cls: Description of cls.
            tool_results: Description of tool_results.
...
def _format_summary_message(index: int, message: Message) -> str:
    """Implements  format summary message.
...
def _format_messages_for_summary(messages: list[Message], *, start_index: int = 1) -> str:
    """Implements  format messages for summary.
...
def _select_summary_input_groups(
    ...
    *,
    ...
    """Implements  select summary input groups.
    
    Args:
        groups: Description of groups.
        prompt: Description of prompt.
        max_summary_input_tokens: Description of max_summary_input_tokens.
...
class SummarizationStrategy:
    ...
    def _record_summary_failure(self) -> None:
        """Implements  record summary failure.
    ...
    def _record_summary_success(self) -> None:
        """Implements  record summary success.
