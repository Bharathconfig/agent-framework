...
class _FakeMontyComplete:
    """Represents the _FakeMontyComplete type and related behavior.
    """
    output: Any = None
...
class _FakeFunctionSnapshot:
    """Represents the _FakeFunctionSnapshot type and related behavior.
    """
    function_name: str
    call_id: int
    args: tuple[Any, ...] = ()
...
class _FakeFutureSnapshot:
    """Represents the _FakeFutureSnapshot type and related behavior.
    """
...
class _FakeNameLookupSnapshot:
    """Represents the _FakeNameLookupSnapshot type and related behavior.
    """
    variable_name: str
...
class _FakeMonty:
    """Represents the _FakeMonty type and related behavior.
    """
    def __init__(
        self,
        code: str,
        *,
        ...
        Args:
            ...
            script_name: Description of script_name.
            type_check: Description of type_check.
            type_check_stubs: Description of type_check_stubs.
        """
        self.code = code
    ...
    def start(self, *, print_callback: Any) -> Any:
        ...
        Args:
            self: Description of self.
            print_callback: Description of print_callback.
...
def test_tool_construction_defaults() -> None:
    """Validates behavior for tool construction defaults.
    """
...
def test_add_remove_clear_tools_round_trip() -> None:
    """Validates behavior for add remove clear tools round trip.
    """
    monty_tool = MontyExecuteCodeTool()
...
def test_approval_required_tool_gates_execute_code() -> None:
    """Validates behavior for approval required tool gates execute code.
    """
...
def test_default_approval_mode_always_require_is_sticky() -> None:
    """Validates behavior for default approval mode always require is sticky.
    """
...
def test_dynamic_description_reflects_registered_tools() -> None:
    """Validates behavior for dynamic description reflects registered tools.
    """
    monty_tool = MontyExecuteCodeTool(tools=[add_tool])
...
def test_create_run_tool_snapshots_current_state() -> None:
    """Validates behavior for create run tool snapshots current state.
    """
...
def test_build_serializable_state_matches_effective_config() -> None:
    """Validates behavior for build serializable state matches effective config.
    """
...
def test_file_mounts_normalized_and_round_tripped(tmp_path: Path) -> None:
    """Validates behavior for file mounts normalized and round tripped.
...
def test_workspace_root_auto_mounts_at_input(tmp_path: Path) -> None:
    """Validates behavior for workspace root auto mounts at input.
...
def test_workspace_root_yields_to_explicit_input_mount(tmp_path: Path) -> None:
    """Validates behavior for workspace root yields to explicit input mount.
...
def test_remove_file_mount_raises_on_missing() -> None:
    """Validates behavior for remove file mount raises on missing.
    """
    monty_tool = MontyExecuteCodeTool()
    with pytest.raises(KeyError):
...
def test_dynamic_description_mentions_filesystem_when_mounts_configured(tmp_path: Path) -> None:
    """Validates behavior for dynamic description mentions filesystem when mounts configured.
...
def test_dynamic_description_default_mentions_no_filesystem() -> None:
    """Validates behavior for dynamic description default mentions no filesystem.
    """
    monty_tool = MontyExecuteCodeTool()
    description = monty_tool.description
...
def test_instruction_builders_describe_write_caps_and_visible_tools(tmp_path: Path) -> None:
    """Validates behavior for instruction builders describe write caps and visible tools.
...
def test_resource_limits_round_trip() -> None:
    """Validates behavior for resource limits round trip.
    """
...
def test_build_instructions_includes_registered_tools() -> None:
    """Validates behavior for build instructions includes registered tools.
    """
    monty_tool = MontyExecuteCodeTool(tools=[add_tool])
...
def test_execute_code_filtered_out_when_added_as_tool() -> None:
    """Validates behavior for execute code filtered out when added as tool.
    """
...
def test_mount_helpers_validate_inputs_and_convert_mounts(
    ...
    """Validates behavior for mount helpers validate inputs and convert mounts.
    ...
    class _FakeMountDir:
        """Represents the _FakeMountDir type and related behavior.
        """
...
def test_to_dict_materializes_dynamic_description(tmp_path: Path) -> None:
    """Validates behavior for to dict materializes dynamic description.
...
async def test_run_code_with_no_tools_returns_default_text() -> None:
    """Validates behavior for run code with no tools returns default text.
    """
...
async def test_run_code_surfaces_stdout_and_output() -> None:
    """Validates behavior for run code surfaces stdout and output.
    """
...
async def test_run_code_direct_typed_call_invokes_registered_tool() -> None:
    """Validates behavior for run code direct typed call invokes registered tool.
    """
...
async def test_run_code_call_tool_fallback_invokes_registered_tool() -> None:
    """Validates behavior for run code call tool fallback invokes registered tool.
    """
    func_snapshot = _FakeFunctionSnapshot(
...
async def test_run_code_unknown_tool_returns_nameerror_resume() -> None:
    """Validates behavior for run code unknown tool returns nameerror resume.
    """
    func_snapshot = _FakeFunctionSnapshot(
...
async def test_run_code_os_function_is_rejected_with_permissionerror() -> None:
    """Validates behavior for run code os function is rejected with permissionerror.
    """
...
async def test_run_code_call_tool_with_unregistered_name_returns_error() -> None:
    """Validates behavior for run code call tool with unregistered name returns error.
    """
    func_snapshot = _FakeFunctionSnapshot(
        function_name="call_tool",
        call_id=20,
...
async def test_run_code_returns_error_content_on_runtime_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for run code returns error content on runtime failure.
    
    Args:
        monkeypatch: Description of monkeypatch.
    """
    class _BoomBridge:
        """Represents the _BoomBridge type and related behavior.
        """
        def __init__(self, tool_map: Any, **_: Any) -> None:
            ...
            Args:
                self: Description of self.
                tool_map: Description of tool_map.
                **_: Description of _.
            """
            pass
...
def test_build_execution_contents_handles_truncation_and_non_json_output() -> None:
    """Validates behavior for build execution contents handles truncation and non json output.
    """
...
def test_capture_written_files_returns_new_files_and_omits_large_ones(
    ...
    """Validates behavior for capture written files returns new files and omits large ones.
...
async def test_provider_injects_execute_code_tool_and_instructions() -> None:
    """Validates behavior for provider injects execute code tool and instructions.
    """
...
def test_provider_delegates_tool_management_to_internal_tool() -> None:
    """Validates behavior for provider delegates tool management to internal tool.
    """
...
def test_provider_delegates_file_mount_management_to_internal_tool(tmp_path: Path) -> None:
    """Validates behavior for provider delegates file mount management to internal tool.
...
def test_generate_type_stubs_emits_dsl_and_tool_signatures() -> None:
    """Validates behavior for generate type stubs emits dsl and tool signatures.
...
def test_generate_type_stubs_preserves_none_and_optional() -> None:
    ...
    """Validates behavior for generate type stubs preserves none and optional.
