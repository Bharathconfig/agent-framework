"""This module defines functionality for packages/foundry/tests/foundry/test_foundry_chat_client.
...
def test_foundry_feature_usage_policy_refreshes_user_agent() -> None:
    """Validates behavior for foundry feature usage policy refreshes user agent.
    """
...
def test_foundry_feature_usage_policy_removes_token_on_cross_origin_redirect() -> None:
    """Validates behavior for foundry feature usage policy removes token on cross origin redirect.
    
    Returns:
        Description of the return value.
    """
    class _Response(HttpResponse):
        """Represents the _Response type and related behavior.
        """
        def body(self) -> bytes:
            """Implements body.
    ...
    class _RedirectTransport(HttpTransport[HttpRequest, HttpResponse]):
        """Represents the _RedirectTransport type and related behavior.
        """
...
def test_foundry_feature_index_does_not_own_toolbox() -> None:
    """Validates behavior for foundry feature index does not own toolbox.
    """
    assert not hasattr(FeatureIndex, "FOUNDRY_TOOLBOX")


def test_raw_foundry_chat_client_owns_foundry_feature_bit() -> None:
    """Validates behavior for raw foundry chat client owns foundry feature bit.
    """
...
def _make_mock_openai_client() -> MagicMock:
    """Implements  make mock openai client.
...
def test_raw_foundry_chat_client_init_uses_explicit_parameters() -> None:
    """Validates behavior for raw foundry chat client init uses explicit parameters.
    """
...
def test_foundry_chat_client_init_uses_explicit_parameters() -> None:
    """Validates behavior for foundry chat client init uses explicit parameters.
    """
...
def test_init_with_project_endpoint_creates_project_client() -> None:
    """Validates behavior for init with project endpoint creates project client.
    """
...
def test_init_with_empty_model_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for init with empty model raises.
...
def test_init_with_empty_project_source_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for init with empty project source raises.
...
def test_init_with_project_endpoint_requires_credential() -> None:
    """Validates behavior for init with project endpoint requires credential.
    """
...
async def test_configure_azure_monitor() -> None:
    """Validates behavior for configure azure monitor.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
async def test_configure_azure_monitor_resource_not_found() -> None:
    """Validates behavior for configure azure monitor resource not found.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
async def test_configure_azure_monitor_import_error() -> None:
    """Validates behavior for configure azure monitor import error.
    ...
    def _import_with_missing_azure_monitor(
        ...
        """Implements  import with missing azure monitor.
        
        Args:
            name: Description of name.
            globals: Description of globals.
            locals: Description of locals.
...
async def test_configure_azure_monitor_with_custom_resource() -> None:
    """Validates behavior for configure azure monitor with custom resource.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
async def test_get_response_with_invalid_input() -> None:
    """Validates behavior for get response with invalid input.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
async def test_web_search_tool_with_location() -> None:
    """Validates behavior for web search tool with location.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
async def test_code_interpreter_tool_variations() -> None:
    """Validates behavior for code interpreter tool variations.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
async def test_hosted_file_search_tool_validation() -> None:
    """Validates behavior for hosted file search tool validation.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
async def test_chat_message_parsing_with_function_calls() -> None:
    """Validates behavior for chat message parsing with function calls.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
def test_foundry_function_result_list_respects_rich_output_capability(
    ...
    """Validates behavior for foundry function result list respects rich output capability.
...
async def test_content_filter_exception() -> None:
    """Validates behavior for content filter exception.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
async def test_response_format_parse_path() -> None:
    """Validates behavior for response format parse path.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
async def test_response_format_parse_path_with_conversation_id() -> None:
    """Validates behavior for response format parse path with conversation id.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
async def test_response_format_dict_parse_path() -> None:
    """Validates behavior for response format dict parse path.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
async def test_bad_request_error_non_content_filter() -> None:
    """Validates behavior for bad request error non content filter.
    """
    mock_openai_client = _make_mock_openai_client()
    project_client = MagicMock()
    project_client.get_openai_client.return_value = mock_openai_client
...
def test_get_mcp_tool_with_project_connection_id() -> None:
    """Validates behavior for get mcp tool with project connection id.
    """
...
async def test_integration_options(
    ...
    """Validates behavior for integration options.
    
    Args:
        option_name: Description of option_name.
        option_value: Description of option_value.
        needs_validation: Description of needs_validation.
    """
...
async def test_integration_tool_rich_content_image() -> None:
    """Validates behavior for integration tool rich content image.
...
def test_agent_accepts_foundry_chat_clients() -> None:
    """Validates behavior for agent accepts foundry chat clients.
    """
    mock_project = MagicMock()
    mock_openai = _make_mock_openai_client()
    mock_project.get_openai_client.return_value = mock_openai
...
def _concurrency_response(marker: str, model: str) -> dict[str, Any]:
    """Implements  concurrency response.
...
def _concurrency_stream_events(marker: str, model: str) -> list[dict[str, Any]]:
    """Implements  concurrency stream events.
...
class _ResponsesTransport:
    """Represents the _ResponsesTransport type and related behavior.
    """
...
class _ProjectClient:
    """Represents the _ProjectClient type and related behavior.
    """
...
def _build_concurrency_client(
    ...
    """Implements  build concurrency client.
...
async def _run_concurrent_agent(
    ...
    *,
    ...
    """Implements  run concurrent agent.
...
async def test_shared_chat_client_keeps_concurrent_agent_runs_isolated(
    ...
    """Validates behavior for shared chat client keeps concurrent agent runs isolated.
    
    Args:
        provider: Description of provider.
        first_stream: Description of first_stream.
        second_stream: Description of second_stream.
    """
    model = "test-model"
