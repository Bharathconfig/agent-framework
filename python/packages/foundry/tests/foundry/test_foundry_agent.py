"""This module defines functionality for packages/foundry/tests/foundry/test_foundry_agent.
...
def test_raw_foundry_agent_chat_client_does_not_mark_openai_feature() -> None:
    """Validates behavior for raw foundry agent chat client does not mark openai feature.
    """
...
def test_raw_foundry_agent_chat_client_creates_project_client_with_feature_policy() -> None:
    """Validates behavior for raw foundry agent chat client creates project client with feature policy.
    """
    mock_project = MagicMock()
    mock_project.get_openai_client.return_value = MagicMock()
...
def test_agent_accepts_raw_foundry_agent_chat_client() -> None:
    """Validates behavior for agent accepts raw foundry agent chat client.
    """
    mock_project = MagicMock()
    mock_project.get_openai_client.return_value = MagicMock()
...
def test_raw_foundry_agent_chat_client_init_uses_explicit_parameters() -> None:
    """Validates behavior for raw foundry agent chat client init uses explicit parameters.
    """
...
def test_raw_foundry_agent_chat_client_as_agent_preserves_client_type() -> None:
    ...
    class CustomClient(RawFoundryAgentChatClient):
        """Represents the CustomClient type and related behavior.
        """
        pass
...
def test_raw_foundry_agent_chat_client_as_agent_uses_explicit_parameters() -> None:
    """Validates behavior for raw foundry agent chat client as agent uses explicit parameters.
    """
...
def test_foundry_agent_chat_client_init_uses_explicit_parameters() -> None:
    """Validates behavior for foundry agent chat client init uses explicit parameters.
    """
...
def test_raw_foundry_agent_init_uses_explicit_parameters() -> None:
    """Validates behavior for raw foundry agent init uses explicit parameters.
    """
...
def test_foundry_agent_init_uses_explicit_parameters() -> None:
    """Validates behavior for foundry agent init uses explicit parameters.
    """
...
def test_foundry_agent_init_with_middleware() -> None:
    ...
    class MyMiddleware(ChatMiddleware):
        """Represents the MyMiddleware type and related behavior.
        """
...
async def test_foundry_agent_workflow_replays_parallel_reasoning_function_group(
    ...
    async def foundry_responses_boundary(request: httpx.Request) -> httpx.Response:
        """Implements foundry responses boundary.
    ...
    class TerminateToolLoopMiddleware(FunctionMiddleware):
        """Represents the TerminateToolLoopMiddleware type and related behavior.
        """
        async def process(
            self,
...
async def test_foundry_agent_ag_ui_service_session_continues_without_history_replay(
    ...
    async with FoundryAgent(credential=cast(Any, AzureCliCredential()), allow_preview=True) as foundry_agent:
        ...
        def capture_provider_input(messages: Any = None, **kwargs: Any) -> Any:
            """Implements capture provider input.
