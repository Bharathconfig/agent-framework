...
class _FakeAccessToken:
    """Represents the _FakeAccessToken type and related behavior.
    """
...
def test_resolve_endpoint_prefers_explicit_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for resolve endpoint prefers explicit env.
...
def test_resolve_endpoint_builds_from_project_and_name(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for resolve endpoint builds from project and name.
...
def test_resolve_endpoint_empty_explicit_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for resolve endpoint empty explicit raises.
...
def test_resolve_endpoint_missing_inputs_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for resolve endpoint missing inputs raises.
...
def test_toolbox_name_from_endpoint(endpoint: str, expected: str) -> None:
    """Validates behavior for toolbox name from endpoint.
...
def test_init_derives_name_and_defaults() -> None:
    """Validates behavior for init derives name and defaults.
    """
...
def test_init_forwards_additional_tool_arguments_and_parent_kwargs() -> None:
    """Validates behavior for init forwards additional tool arguments and parent kwargs.
    """
    toolbox = FoundryToolbox(
        _FakeCredential(),  # type: ignore
        url="https://h/toolboxes/sales/mcp?api-version=v1",
...
def test_toolbox_owns_feature_index_53() -> None:
    """Validates behavior for toolbox owns feature index 53.
    """
    assert FeatureIndex.FOUNDRY_TOOLBOX == 53


async def test_toolbox_marks_feature_on_successful_connect_not_construction() -> None:
    """Validates behavior for toolbox marks feature on successful connect not construction.
    """
    with (
...
async def test_auth_flow_injects_bearer_token() -> None:
    """Validates behavior for auth flow injects bearer token.
    """
...
async def test_auth_flow_injects_bearer_token_async_credential() -> None:
    """Validates behavior for auth flow injects bearer token async credential.
    """
...
def test_sync_auth_flow_injects_bearer_token() -> None:
    """Validates behavior for sync auth flow injects bearer token.
    """
...
def test_sync_auth_flow_rejects_async_credential() -> None:
    """Validates behavior for sync auth flow rejects async credential.
    """
...
async def test_auth_flow_forwards_call_id_when_present() -> None:
    """Validates behavior for auth flow forwards call id when present.
    """
...
async def test_auth_flow_omits_call_id_when_absent() -> None:
    """Validates behavior for auth flow omits call id when absent.
    """
    auth = _ToolboxAuth(_FakeCredential(), "scope")  # type: ignore
    request = httpx.Request("POST", "https://h/toolboxes/tb/mcp")
...
async def test_close_closes_owned_http_client() -> None:
    """Validates behavior for close closes owned http client.
    """
    toolbox = FoundryToolbox(
        _FakeCredential(),  # type: ignore
...
def test_as_skills_provider_returns_provider() -> None:
    """Validates behavior for as skills provider returns provider.
    """
    toolbox = FoundryToolbox(
        _FakeCredential(),  # type: ignore
        url="https://h/toolboxes/tb/mcp",
...
def test_as_skills_provider_requires_approval_by_default() -> None:
    """Validates behavior for as skills provider requires approval by default.
    """
    toolbox = FoundryToolbox(
        _FakeCredential(),  # type: ignore
        url="https://h/toolboxes/tb/mcp",
...
def test_as_skills_provider_forwards_approval_overrides() -> None:
    """Validates behavior for as skills provider forwards approval overrides.
    """
    toolbox = FoundryToolbox(
        _FakeCredential(),  # type: ignore
        url="https://h/toolboxes/tb/mcp",
...
async def test_skills_source_requires_connection() -> None:
    """Validates behavior for skills source requires connection.
    """
    toolbox = FoundryToolbox(
        _FakeCredential(),  # type: ignore
        url="https://h/toolboxes/tb/mcp",
...
async def test_skills_source_uses_connected_session(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for skills source uses connected session.
    ...
    class _StubSkillsSource:
        """Represents the _StubSkillsSource type and related behavior.
        """
        def __init__(self, *, session_provider: Callable[[], object], **kwargs: object) -> None:
            ...
            Args:
                self: Description of self.
                session_provider: Description of session_provider.
                **kwargs: Description of kwargs.
            """
...
async def test_skills_source_forwards_archive_options(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for skills source forwards archive options.
...
async def test_skills_source_requires_connection_via_provider() -> None:
    """Validates behavior for skills source requires connection via provider.
    """
    toolbox = FoundryToolbox(
        _FakeCredential(),  # type: ignore
        url="https://h/toolboxes/tb/mcp",
...
def _patch_counting_mcp_source(monkeypatch: pytest.MonkeyPatch) -> list[int]:
    ...
    class _CountingSkillsSource:
        """Represents the _CountingSkillsSource type and related behavior.
        """
...
async def test_as_skills_provider_caches_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for as skills provider caches by default.
...
async def test_as_skills_provider_disable_caching_rereads_every_run(monkeypatch: pytest.MonkeyPatch) -> None:
    """Validates behavior for as skills provider disable caching rereads every run.
...
async def test_as_skills_provider_cache_refresh_interval_rereads_after_staleness(
    ...
    """Validates behavior for as skills provider cache refresh interval rereads after staleness.
...
def test_as_skills_provider_forwards_only_set_archive_options() -> None:
    """Validates behavior for as skills provider forwards only set archive options.
    """
    toolbox = FoundryToolbox(
        _FakeCredential(),  # type: ignore
        url="https://h/toolboxes/tb/mcp",
...
class TestFoundryToolboxReconnection:
    """Represents the TestFoundryToolboxReconnection type and related behavior.
    """
