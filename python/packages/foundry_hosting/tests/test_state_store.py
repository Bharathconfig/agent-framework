"""This module defines functionality for packages/foundry_hosting/tests/test_state_store.
...
def _config(*, is_hosted: bool) -> AgentConfig:
    ...
    Args:
        is_hosted: Description of is_hosted.
...
def test_storage_providers_use_public_abstraction() -> None:
    """Validates behavior for storage providers use public abstraction.
    """
...
async def test_save_uses_context_scoped_store() -> None:
    """Validates behavior for save uses context scoped store.
    """
    store = _store()
...
async def test_list_checkpoints_restricts_checkpoint_deserialization() -> None:
    """Validates behavior for list checkpoints restricts checkpoint deserialization.
    """
    store = _store()
    checkpoint = _checkpoint("checkpoint-1")
    value = checkpoint.to_dict()
...
async def test_load_raises_for_missing_checkpoint() -> None:
    """Validates behavior for load raises for missing checkpoint.
    """
    store = _store()
...
async def test_list_checkpoints_paginates_and_filters_by_workflow() -> None:
    """Validates behavior for list checkpoints paginates and filters by workflow.
    """
    store = _store()
...
async def test_delete_reports_whether_checkpoint_existed(deleted_id: str | None, expected: bool) -> None:
    """Validates behavior for delete reports whether checkpoint existed.
    
    Args:
        deleted_id: Description of deleted_id.
...
async def test_get_latest_uses_timestamp_and_list_ids_filters() -> None:
    """Validates behavior for get latest uses timestamp and list ids filters.
    """
...
async def test_get_latest_returns_none_when_no_checkpoints_exist() -> None:
    """Validates behavior for get latest returns none when no checkpoints exist.
    """
    storage = FoundryCheckpointStore("context-1", _platform_context())
...
def test_checkpoint_storage_provider_creates_request_scoped_storage(is_hosted: bool) -> None:
    """Validates behavior for checkpoint storage provider creates request scoped storage.
...
def test_checkpoint_stores_require_context_id(create_store: Callable[[], Any]) -> None:
    """Validates behavior for checkpoint stores require context id.
    
    Args:
        create_store: Description of create_store.
    """
...
def _approval_request(approval_request_id: str) -> Content:
    """Implements  approval request.
    
    Args:
        approval_request_id: Description of approval_request_id.
...
async def test_save_and_load_function_approval_request() -> None:
    """Validates behavior for save and load function approval request.
    """
    store = _store()
...
async def test_save_duplicate_function_approval_request_raises() -> None:
    """Validates behavior for save duplicate function approval request raises.
    """
    store = _store()
...
async def test_load_missing_function_approval_request_raises() -> None:
    """Validates behavior for load missing function approval request raises.
    """
    store = _store()
    store.get_item = AsyncMock(return_value=None)
...
def test_function_approval_storage_provider_uses_foundry_store(is_hosted: bool) -> None:
    """Validates behavior for function approval storage provider uses foundry store.
...
def test_function_approval_storage_provider_creates_request_scoped_storage() -> None:
    """Validates behavior for function approval storage provider creates request scoped storage.
    """
...
async def test_set_agent_session_uses_scoped_store() -> None:
    """Validates behavior for set agent session uses scoped store.
    """
    store = _store()
...
async def test_get_agent_session_returns_deserialized_session() -> None:
    """Validates behavior for get agent session returns deserialized session.
    """
    store = _store()
    session = AgentSession(session_id="agent-session-1")
    session.state["turn_count"] = 2
...
async def test_get_missing_agent_session_returns_none() -> None:
    """Validates behavior for get missing agent session returns none.
    """
    store = _store()
    store.get_item = AsyncMock(return_value=None)
...
async def test_delete_agent_session_is_idempotent() -> None:
    """Validates behavior for delete agent session is idempotent.
    """
    store = _store()
...
def test_agent_session_storage_provider_uses_foundry_store(is_hosted: bool) -> None:
    """Validates behavior for agent session storage provider uses foundry store.
...
def test_agent_session_storage_provider_creates_request_scoped_storage() -> None:
    """Validates behavior for agent session storage provider creates request scoped storage.
    """
