"""This module defines functionality for packages/hosting-mcp/tests/hosting_mcp/test_agent_tool.
...
async def test_agent_tool_generates_schema_from_agent_with_overrides() -> None:
    """Validates behavior for agent tool generates schema from agent with overrides.
    """
    agent = Agent(
...
async def test_agent_tool_uses_agent_metadata_by_default() -> None:
    """Validates behavior for agent tool uses agent metadata by default.
    """
...
async def test_agent_tool_runs_with_agent_state_session() -> None:
    """Validates behavior for agent tool runs with agent state session.
    """
    client = RecordingClient()
    agent = Agent(
        client=client,
...
def test_agent_tool_rejects_undefined_session_parameter() -> None:
    """Validates behavior for agent tool rejects undefined session parameter.
    """
...
async def test_agent_tool_always_requires_session_parameter() -> None:
    """Validates behavior for agent tool always requires session parameter.
    """
    agent = Agent(client=RecordingClient(), name="agent")
    tool: AgentMCPTool[Any] = AgentMCPTool(
        agent,
