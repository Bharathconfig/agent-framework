"""This module defines functionality for packages/hosting-mcp/tests/hosting_mcp/test_conversion.
...
def test_mcp_to_run_converts_selected_argument() -> None:
    """Validates behavior for mcp to run converts selected argument.
    """
    arguments = {
        "prompt": "hello",
        "temperature": 0.2,
...
def test_mcp_to_run_rejects_missing_argument() -> None:
    """Validates behavior for mcp to run rejects missing argument.
    """
...
def test_mcp_to_run_rejects_non_string_argument() -> None:
    """Validates behavior for mcp to run rejects non string argument.
    """
...
def test_mcp_from_run_converts_final_response() -> None:
    """Validates behavior for mcp from run converts final response.
    """
...
def test_mcp_from_run_uses_app_owned_binary_resource_uri() -> None:
    """Validates behavior for mcp from run uses app owned binary resource uri.
    """
    blocks = mcp_from_run(
        Message(
            "assistant",
...
def test_mcp_from_run_preserves_empty_text() -> None:
    """Validates behavior for mcp from run preserves empty text.
    """
...
def test_mcp_from_run_omits_content_not_supported_in_tool_results() -> None:
    """Validates behavior for mcp from run omits content not supported in tool results.
    """
    blocks = mcp_from_run(
        Message(
            "assistant",
...
def test_mcp_from_run_omits_user_messages() -> None:
    """Validates behavior for mcp from run omits user messages.
    """
    assert mcp_from_run(AgentResponse(messages=[Message("user", ["omit me"])])) == []


def test_mcp_from_run_rejects_invalid_data_uri() -> None:
    """Validates behavior for mcp from run rejects invalid data uri.
    """
...
def test_mcp_from_run_rejects_invalid_base64_data() -> None:
    """Validates behavior for mcp from run rejects invalid base64 data.
    """
    content = Content(
        "data",
